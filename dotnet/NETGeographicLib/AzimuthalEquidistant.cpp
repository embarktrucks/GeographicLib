...
AzimuthalEquidistant::AzimuthalEquidistant(void) {
  try {
    m_pAzimuthalEquidistant =
      new GeographicLib::AzimuthalEquidistant(GeographicLib::Geodesic::WGS84());
  } catch (std::bad_alloc) {
...
AzimuthalEquidistant::AzimuthalEquidistant(Geodesic ^ g) {
  try {
    ...
    m_pAzimuthalEquidistant =
      new GeographicLib::AzimuthalEquidistant(*pGeodesic);
  } catch (std::bad_alloc) {
    throw gcnew GeographicErr(BADALLOC);
  }
}
...
//*****************************************************************************
void AzimuthalEquidistant::Forward(double lat0, double lon0, double lat,
                                   double lon, double % x, double % y,
                                   double % azi, double % rk) {
  double lx, ly, lazi, lrk;
  m_pAzimuthalEquidistant->Forward(lat0, lon0, lat, lon, lx, ly, lazi, lrk);
  x = lx;
  y = ly;
  azi = lazi;
  rk = lrk;
...
void AzimuthalEquidistant::Reverse(double lat0, double lon0, double x, double y,
  ...
  m_pAzimuthalEquidistant->Reverse(lat0, lon0, x, y, llat, llon, lazi, lrk);
  lat = llat;
  lon = llon;
  azi = lazi;
  rk = lrk;
