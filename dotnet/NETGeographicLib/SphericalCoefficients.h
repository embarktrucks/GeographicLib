...
GeographicLib::SphericalEngine::coeff object.  The
GeographicLib::SphericalEngine class is not implemented in NETGeographicLib.
SphericalCoefficients is provided as a substitute for
GeographicLib::SphericalEngine::coeff allowing you to examine the coefficients
in .NET applications.
...
ref class SphericalCoefficients {
  ...
  */
   ...
   **********************************************************************/
  int index(int n, int m) { return m * m_N - m * (m - 1) / 2 + n; }
  /**
   * An element of \e C.
   *
   ...
   *   major order.
   **********************************************************************/
  static int Csize(int N, int M) { return (M + 1) * (2 * N - M + 2) / 2; }
   ...
   *   major order.
   **********************************************************************/
  static int Ssize(int N, int M) { return Csize(N, M) - (N + 1); }
};
}  // namespace NETGeographicLib
