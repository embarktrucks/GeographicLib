...
//*****************************************************************************
void Georef::Forward(double lat, double lon, int prec,
...
void Georef::Reverse(System::String ^ georef, double % lat, double % lon,
    ...
    try {
        ...
        GeographicLib::Georef::Reverse(
            StringConvert::ManagedToUnmanaged(georef), llat, llon, lprec,
            centerp);
        lat = llat;
        lon = llon;
        prec = lprec;
...
double Georef::Resolution(int prec) {
    return GeographicLib::Georef::Resolution(prec);
}

//*****************************************************************************
int Georef::Precision(double res) {
    return GeographicLib::Georef::Precision(res);
}
