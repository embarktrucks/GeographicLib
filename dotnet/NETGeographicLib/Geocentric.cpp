...
Geocentric::Geocentric(void) {
    try {
        m_pGeocentric =
            new GeographicLib::Geocentric(GeographicLib::Geocentric::WGS84());
    } catch (std::bad_alloc) {
...
void Geocentric::Forward(
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++) M[i, j] = lM[3 * i + j];
}
...
System::IntPtr ^ Geocentric::GetUnmanaged() {
    return gcnew System::IntPtr(
        const_cast<void*>(reinterpret_cast<const void*>(m_pGeocentric)));
}

//*****************************************************************************
double Geocentric::MajorRadius::get() { return m_pGeocentric->MajorRadius(); }

//*****************************************************************************
double Geocentric::Flattening::get() { return m_pGeocentric->Flattening(); }
