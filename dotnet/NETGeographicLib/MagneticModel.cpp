...
//*****************************************************************************
MagneticModel::MagneticModel(System::String ^ name, System::String ^ path,
                             Geocentric ^ earth) {
  if (name == nullptr)
    throw gcnew GeographicErr("name cannot be a null pointer.");
  if (path == nullptr)
    throw gcnew GeographicErr("path cannot be a null pointer.");
  if (earth == nullptr)
    throw gcnew GeographicErr("earth cannot be a null pointer.");
  ...
  try {
    ...
    m_pMagneticModel = new GeographicLib::MagneticModel(
      ...
      StringConvert::ManagedToUnmanaged(path), *pGeocentric);
  } catch (std::bad_alloc) {
...
MagneticModel::MagneticModel(System::String ^ name, System::String ^ path) {
  ...
  try {
    m_pMagneticModel =
      new GeographicLib::MagneticModel(StringConvert::ManagedToUnmanaged(name),
...
MagneticCircle ^ MagneticModel::Circle(double t, double lat, double h) {
  try {
    ...
    throw gcnew GeographicErr(
      "Failed to allocate memory for a MagneticCircle in "
      "MagneticModel::Circle");
  }
}

//*****************************************************************************
void MagneticModel::FieldComponents(
...
void MagneticModel::FieldComponents(
  ...
  GeographicLib::MagneticModel::FieldComponents(Bx, By, Bz, Bxt, Byt, Bzt, lh,
                                                lf, ld, li, lht, lft, ldt, lit);
  H = lh;
  F = lf;
  D = ld;
  I = li;
...
System::String ^ MagneticModel::MagneticModelName::get() {
  return StringConvert::UnmanagedToManaged(
    m_pMagneticModel->MagneticModelName());
}

//*****************************************************************************
System::String ^ MagneticModel::MagneticModelDirectory::get() {
  return StringConvert::UnmanagedToManaged(
    m_pMagneticModel->MagneticModelDirectory());
}
...
//*****************************************************************************
double MagneticModel::MinHeight::get() { return m_pMagneticModel->MinHeight(); }

//*****************************************************************************
double MagneticModel::MaxHeight::get() { return m_pMagneticModel->MaxHeight(); }
