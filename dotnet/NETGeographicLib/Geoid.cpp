...
//*****************************************************************************
Geoid::Geoid(System::String ^ name, System::String ^ path, bool cubic,
             bool threadsafe) {
  if (name == nullptr)
  ...
  try {
    m_pGeoid = new GeographicLib::Geoid(StringConvert::ManagedToUnmanaged(name),
...
//*****************************************************************************
void Geoid::CacheClear() { m_pGeoid->CacheClear(); }
...
//*****************************************************************************
double Geoid::ConvertHeight(double lat, double lon, double h, ConvertFlag d) {
  try {
    return m_pGeoid->ConvertHeight(
      lat, lon, h, static_cast<GeographicLib::Geoid::convertflag>(d));
  } catch (const std::exception& err) {
    throw gcnew GeographicErr(err.what());
  }
}
