...
LocalCartesian::LocalCartesian(double lat0, double lon0, double h0,
  ...
  try {
    ...
    m_pLocalCartesian =
      new GeographicLib::LocalCartesian(lat0, lon0, h0, *pGeocentric);
  } catch (std::bad_alloc) {
...
LocalCartesian::LocalCartesian(double lat0, double lon0, double h0) {
  try {
    m_pLocalCartesian = new GeographicLib::LocalCartesian(
      lat0, lon0, h0, GeographicLib::Geocentric::WGS84());
  } catch (std::bad_alloc) {
    throw gcnew GeographicErr(BADALLOC);
  }
}
...
LocalCartesian::LocalCartesian() {
  try {
    m_pLocalCartesian =
      new GeographicLib::LocalCartesian(GeographicLib::Geocentric::WGS84());
  } catch (std::bad_alloc) {
    throw gcnew GeographicErr(BADALLOC);
  }
}
