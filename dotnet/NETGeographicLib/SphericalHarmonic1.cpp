...
//*****************************************************************************
SphericalHarmonic1::SphericalHarmonic1(array<double> ^ C, array<double> ^ S,
                                       int N, array<double> ^ C1,
                                       array<double> ^ S1, int N1, double a,
                                       Normalization norm) {
  try {
...
SphericalHarmonic1::SphericalHarmonic1(array<double> ^ C, array<double> ^ S,
                                       ...
                                       int N1, int nmx1, int mmx1, double a,
                                       Normalization norm) {
  try {
    m_C = new std::vector<double>*[m_numCoeffVectors];
    for (int i = 0; i < m_numCoeffVectors; i++)
...
//*****************************************************************************
double SphericalHarmonic1::HarmonicSum(double tau, double x, double y,
                                       double z) {
  return m_pSphericalHarmonic1->operator()(tau, x, y, z);
}

//*****************************************************************************
double SphericalHarmonic1::HarmonicSum(
  double tau, double x, double y, double z,
  ...
  double out =
    m_pSphericalHarmonic1->operator()(tau, x, y, z, lgradx, lgrady, lgradz);
  gradx = lgradx;
  grady = lgrady;
  gradz = lgradz;
  return out;
...
//*****************************************************************************
CircularEngine ^
  SphericalHarmonic1::Circle(double tau, double p, double z, bool gradp) {
  try {
    return gcnew CircularEngine(
      m_pSphericalHarmonic1->Circle(tau, p, z, gradp));
  } catch (std::bad_alloc) {
    throw gcnew GeographicErr(
      "Failed to allocate memory for a CircularEngine in "
      "SphericalHarmonic1::Circle");
  }
}
