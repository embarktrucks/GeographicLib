...
//*****************************************************************************
SphericalHarmonic::SphericalHarmonic(array<double> ^ C, array<double> ^ S,
                                     int N, double a, Normalization norm) {
    try {
        m_C = new std::vector<double>();
        m_S = new std::vector<double>();
        for
            each(double x in C) m_C->push_back(x);
        for
            each(double x in S) m_S->push_back(x);
...
//*****************************************************************************
SphericalHarmonic::SphericalHarmonic(array<double> ^ C, array<double> ^ S,
                                     int N, int nmx, int mmx, double a,
                                     Normalization norm) {
    try {
        m_C = new std::vector<double>();
        m_S = new std::vector<double>();
