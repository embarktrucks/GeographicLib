...
TransverseMercatorExact::TransverseMercatorExact(double a, double f, double k0,
  ...
  try {
    m_pTransverseMercatorExact =
      new GeographicLib::TransverseMercatorExact(a, f, k0, extendp);
  } catch (std::bad_alloc) {
...
//*****************************************************************************
void TransverseMercatorExact::Forward(
  ...
  m_pTransverseMercatorExact->Forward(lon0, lat, lon, lx, ly, lgamma, lk);
  x = lx;
  y = ly;
  gamma = lgamma;
  k = lk;
...
//*****************************************************************************
void TransverseMercatorExact::Reverse(
  ...
  m_pTransverseMercatorExact->Reverse(lon0, x, y, llat, llon, lgamma, lk);
  lat = llat;
  lon = llon;
  gamma = lgamma;
  k = lk;
