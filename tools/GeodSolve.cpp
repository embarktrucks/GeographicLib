...
std::string LatLonString(real lat, real lon, int prec, bool dms, char dmssep,
    ...
    std::string latstr = dms ? DMS::Encode(lat, prec + 5, DMS::LATITUDE, dmssep)
                ...
                lonstr =
                    dms ? DMS::Encode(lon, prec + 5, DMS::LONGITUDE, dmssep)
    ...
    try {
        ...
        for (int m = 1; m < argc; ++m) {
            ...
            if (arg == "-i") {
                ...
                try {
                    DMS::DecodeLatLon(std::string(argv[m + 1]),
                                      std::string(argv[m + 2]), lat1, lon1,
                                      longfirst);
                ...
                try {
                    DMS::DecodeLatLon(std::string(argv[m + 1]),
                                      std::string(argv[m + 2]), lat1, lon1,
                                      longfirst);
                    DMS::DecodeLatLon(std::string(argv[m + 3]),
                                      std::string(argv[m + 4]), lat2, lon2,
                                      longfirst);
                } catch (const std::exception& e) {
        ...
        if (linecalc) {
            ...
            if (exact) {
                le = linecalc == DIRECT
                         ? geode.GenDirectLine(lat1, lon1, azi1, arcmodeline,
                                               s12, outmask)
                         : linecalc == INVERSE
                               ? geode.InverseLine(lat1, lon1, lat2, lon2,
                                                   outmask)
                               :
                               // linecalc == LINE
                ...
                ls = linecalc == DIRECT
                         ? geods.GenDirectLine(lat1, lon1, azi1, arcmodeline,
                                               s12, outmask)
                         : linecalc == INVERSE
                               ? geods.InverseLine(lat1, lon1, lat2, lon2,
                                                   outmask)
                               :
                               // linecalc == LINE
        ...
        while (std::getline(*input, s)) {
            try {
                ...
                if (inverse) {
                    ...
                    a12 = exact ? geode.GenInverse(lat1, lon1, lat2, lon2,
                                                   outmask, s12, azi1, azi2,
                                                   m12, M12, M21, S12)
                                : geods.GenInverse(lat1, lon1, lat2, lon2,
                                                   outmask, s12, azi1, azi2,
                                                   m12, M12, M21, S12);
                    if (full) {
                        if (unroll) {
                    ...
                    *output << eol;
                    ...
                    if (linecalc) {
                        ...
                        a12 = exact ? le.GenPosition(arcmode, s12, outmask,
                                                     lat2, lon2, azi2, s12, m12,
                                                     M12, M21, S12)
                        ...
                        a12 = exact ? geode.GenDirect(lat1, lon1, azi1, arcmode,
                                                      s12, outmask, lat2, lon2,
                                                      azi2, s12, m12, M12, M21,
                                                      S12)
                                    : geods.GenDirect(lat1, lon1, azi1, arcmode,
                                                      s12, outmask, lat2, lon2,
                                                      azi2, s12, m12, M12, M21,
                                                      S12);
                    }
                    ...
                    *output << LatLonString(lat2, lon2, prec, dms, dmssep,
                    ...
                    if (full)
                        *output << " "
                                << DistanceStrings(s12, a12, full, arcmode,
                                                   prec, dms)
