            ...
            if (arg == "-a") {
                ...
                try {
                    DMS::DecodeLatLon(std::string(argv[m + 1]),
                                      std::string(argv[m + 2]), caches, cachew,
                                      longfirst);
                    DMS::DecodeLatLon(std::string(argv[m + 3]),
                                      std::string(argv[m + 4]), cachen, cachee,
                                      longfirst);
                ...
                if (!(zonenum >= UTMUPS::MINZONE &&
                      zonenum <= UTMUPS::MAXZONE)) {
                ...
                int retval =
                    usage(!(arg == "-h" || arg == "--help"), arg != "--help");
                if (arg == "-h")
                    std::cout << "\nDefault geoid path = \""
                              << Geoid::DefaultGeoidPath()
                              << "\"\nDefault geoid name = \""
                    ...
                    if (!cdelim.empty()) {
                        ...
                        if (m != std::string::npos) {
                            ...
                            std::string::size_type m1 =
                                m > 0 ? s.find_last_not_of(spaces, m - 1)
                                      : std::string::npos;
                            s = s.substr(0,
                                         m1 != std::string::npos ? m1 + 1 : m);
                        }
                    }
                    real height = 0;
                    if (zonenum != UTMUPS::INVALID) {
                        ...
                        if (heightmult) {
                            ...
                            std::string::size_type pb =
                                s.find_last_not_of(spaces);
                            std::string::size_type pa =
                                s.find_last_of(spaces, pb);
                            if (pa == std::string::npos ||
                                pb == std::string::npos)
                                throw GeographicErr("Incomplete input: " + s);
                            height =
                                Utility::val<real>(s.substr(pa + 1, pb - pa));
                            s = s.substr(0, pa + 1);
                        }
