        ...
        for (int m = 1; m < argc; ++m) {
            ...
            if (arg == "-n") {
                ...
                try {
                    using std::abs;
                    time =
                        Utility::fractionalyear<real>(std::string(argv[++m]));
                    DMS::flag ind;
                    lat = DMS::Decode(std::string(argv[++m]), ind);
                    if (ind == DMS::LONGITUDE)
                        throw GeographicErr(
                            "Bad hemisphere letter on latitude");
                    if (!(abs(lat) <= 90))
        ...
        try {
            ...
            if (verbose) {
                std::cerr << "Magnetic file: " << m.MagneticFile() << "\n"
                          ...
                          << "Time range: [" << m.MinTime() << ","
                          << m.MaxTime() << "]\n"
                          << "Height range: [" << m.MinHeight() / 1000 << "km,"
                          << m.MaxHeight() / 1000 << "km]\n";
            }
            if ((timeset || circle) &&
                (time < m.MinTime() || time > m.MaxTime()))
            ...
            while (std::getline(*input, s)) {
                try {
                    ...
                    if (circle) {
                        ...
                        if (ind == DMS::LATITUDE)
                            throw GeographicErr("Bad hemisphere letter on " +
                                                strb);
                    } else {
                        if (!(str >> stra >> strb))
                            throw GeographicErr("Incomplete input: " + s);
                    ...
                    MagneticModel::FieldComponents(bx, by, bz, bxt, byt, bzt, H,
                                                   F, D, I, Ht, Ft, Dt, It);
