...
int main(int argc, const char* const argv[]) {
    ...
    try {
        ...
        for (int ilat0 = 0; ilat0 < nlat;
             ilat0 += nbatch) {  // Loop over batches
            int nlat0 = min(nlat, ilat0 + nbatch);

#if HAVE_OPENMP
#pragma omp parallel for
#endif
            for (int ilat = ilat0; ilat < nlat0;
                 ++ilat) {                     // Loop over latitudes
                ...
                for (int ilon = 0; ilon < nlon;
                     ++ilon) {  // Loop over longitudes
                    Math::real lon = lonorg + (ilon / ndeg) +
