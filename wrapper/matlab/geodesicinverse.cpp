...
void compute(double a, double f, mwSize m, const double* latlong,
    ...
    for (mwIndex i = 0; i < m; ++i) {
        if (abs(lat1[i]) <= 90 && lon1[i] >= -540 && lon1[i] < 540 &&
            ...
            if (aux)
                a12[i] =
                    g.Inverse(lat1[i], lon1[i], lat2[i], lon2[i], s12[i],
                              azi1[i], azi2[i], m12[i], M12[i], M21[i], S12[i]);
            else
                g.Inverse(lat1[i], lon1[i], lat2[i], lon2[i], s12[i], azi1[i],
                          azi2[i]);
        }
    }
}
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
    ...
    double* aux = nlhs == 2
                      ? mxGetPr(plhs[1] = mxCreateDoubleMatrix(m, 5, mxREAL))
                      : NULL;
