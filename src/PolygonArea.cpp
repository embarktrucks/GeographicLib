...
void PolygonAreaT<GeodType>::AddEdge(real azi, real s) {
  if (_num) {  // Do nothing if _num is zero
    real lat, lon, S12, t;
    _earth.GenDirect(_lat1, _lon1, azi, false, s, _mask, lat, lon, t, t, t, t,
                     t, S12);
    _perimetersum += s;
    if (!_polyline) {
      _areasum += S12;
...
unsigned PolygonAreaT<GeodType>::Compute(bool reverse, bool sign,
  ...
  _earth.GenInverse(_lat1, _lon1, _lat0, _lon0, _mask, s12, t, t, t, t, t, S12);
...
unsigned PolygonAreaT<GeodType>::TestPoint(real lat, real lon, bool reverse,
  ...
  for (int i = 0; i < (_polyline ? 1 : 2); ++i) {
    ...
    _earth.GenInverse(i == 0 ? _lat1 : lat, i == 0 ? _lon1 : lon,
                      i != 0 ? _lat0 : lat, i != 0 ? _lon0 : lon, _mask, s12, t,
                      t, t, t, t, S12);
    perimeter += s12;
    if (!_polyline) {
      tempsum += S12;
