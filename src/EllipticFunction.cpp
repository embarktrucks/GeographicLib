...
Math::real EllipticFunction::RF(real x, real y, real z) {
    ...
    while (Q >= mul * abs(An)) {
        // Max 6 trips
        real lam =
            sqrt(x0) * sqrt(y0) + sqrt(y0) * sqrt(z0) + sqrt(z0) * sqrt(x0);
        An = (An + lam) / 4;
        x0 = (x0 + lam) / 4;
        y0 = (y0 + lam) / 4;
        z0 = (z0 + lam) / 4;
...
Math::real EllipticFunction::RJ(real x, real y, real z, real p) {
    ...
    static const real tolRD =
        pow(real(0.2) * (numeric_limits<real>::epsilon() * real(0.01)),
            1 / real(8));
    ...
    while (Q >= mul * abs(An)) {
        // Max 7 trips
        real lam = sqrt(x0) * sqrt(y0) + sqrt(y0) * sqrt(z0) +
                   sqrt(z0) * sqrt(x0),
