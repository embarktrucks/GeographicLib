...
Math::real EllipticFunction::RF(real x, real y, real z) {
  ...
  real A0 = (x + y + z) / 3, An = A0,
       Q = max(max(abs(A0 - x), abs(A0 - y)), abs(A0 - z)) / tolRF, x0 = x,
       y0 = y, z0 = z, mul = 1;
  while (Q >= mul * abs(An)) {
    // Max 6 trips
  ...
  real X = (A0 - x) / (mul * An), Y = (A0 - y) / (mul * An), Z = -(X + Y),
       E2 = X * Y - Z * Z, E3 = X * Y * Z;
...
Math::real EllipticFunction::RG(real x, real y) {
  ...
  static const real tolRG0 =
    real(2.7) * sqrt((numeric_limits<real>::epsilon() * real(0.01)));
  real x0 = sqrt(max(x, y)), y0 = sqrt(min(x, y)), xn = x0, yn = y0, s = 0,
       mul = real(0.25);
  while (abs(xn - yn) > tolRG0 * xn) {
    // Max 4 trips
    real t = (xn + yn) / 2;
...
Math::real EllipticFunction::RJ(real x, real y, real z, real p) {
  ...
  static const real tolRD = pow(
    real(0.2) * (numeric_limits<real>::epsilon() * real(0.01)), 1 / real(8));
  real A0 = (x + y + z + 2 * p) / 5, An = A0,
       delta = (p - x) * (p - y) * (p - z),
       Q = max(max(abs(A0 - x), abs(A0 - y)), max(abs(A0 - z), abs(A0 - p))) /
           tolRD,
       x0 = x, y0 = y, z0 = z, p0 = p, mul = 1, mul3 = 1, s = 0;
  while (Q >= mul * abs(An)) {
    // Max 7 trips
    real lam = sqrt(x0) * sqrt(y0) + sqrt(y0) * sqrt(z0) + sqrt(z0) * sqrt(x0),
         d0 = (sqrt(p0) + sqrt(x0)) * (sqrt(p0) + sqrt(y0)) *
              (sqrt(p0) + sqrt(z0)),
  ...
  real X = (A0 - x) / (mul * An), Y = (A0 - y) / (mul * An),
       Z = (A0 - z) / (mul * An), P = -(X + Y + Z) / 2,
       E2 = X * Y + X * Z + Y * Z - 3 * P * P,
       E3 = X * Y * Z + 2 * P * (E2 + 2 * P * P),
       E4 = (2 * X * Y * Z + P * (E2 + 3 * P * P)) * P, E5 = X * Y * Z * P * P;
...
Math::real EllipticFunction::RD(real x, real y, real z) {
  ...
  static const real tolRD = pow(
    real(0.2) * (numeric_limits<real>::epsilon() * real(0.01)), 1 / real(8));
  real A0 = (x + y + 3 * z) / 5, An = A0,
       Q = max(max(abs(A0 - x), abs(A0 - y)), abs(A0 - z)) / tolRD, x0 = x,
       y0 = y, z0 = z, mul = 1, s = 0;
  while (Q >= mul * abs(An)) {
    // Max 7 trips
    real lam = sqrt(x0) * sqrt(y0) + sqrt(y0) * sqrt(z0) + sqrt(z0) * sqrt(x0);
  ...
  real X = (A0 - x) / (mul * An), Y = (A0 - y) / (mul * An), Z = -(X + Y) / 3,
       E2 = X * Y - 6 * Z * Z, E3 = (3 * X * Y - 8 * Z * Z) * Z,
       E4 = 3 * (X * Y - Z * Z) * Z * Z, E5 = X * Y * Z * Z * Z;
  // http://dlmf.nist.gov/19.36.E2
  // Polynomial is
  // (1 - 3*E2/14 + E3/6 + 9*E2^2/88 - 3*E4/22 - 9*E2*E3/52 + 3*E5/26
  //    - E2^3/16 + 3*E3^2/40 + 3*E2*E4/20 + 45*E2^2*E3/272
...
void EllipticFunction::Reset(real k2, real alpha2, real kp2, real alphap2) {
  // Accept nans here (needed for GeodesicExact)
  if (k2 > 1) throw GeographicErr("Parameter k2 is not in (-inf, 1]");
  if (alpha2 > 1) throw GeographicErr("Parameter alpha2 is not in (-inf, 1]");
  if (kp2 < 0) throw GeographicErr("Parameter kp2 is not in [0, inf)");
  if (alphap2 < 0) throw GeographicErr("Parameter alphap2 is not in [0, inf)");
  _k2 = k2;
  _kp2 = kp2;
  _alpha2 = alpha2;
  _alphap2 = alphap2;
  ...
  if (_alpha2 != 0) {
    ...
    real rj = (_kp2 != 0 && _alphap2 != 0) ? RJ(0, _kp2, 1, _alphap2)
      ...
      rc = _kp2 != 0 ? 0 : (_alphap2 != 0 ? RC(1, _alphap2) : Math::infinity());
    // Pi(alpha^2, k)
...
void EllipticFunction::sncndn(real x, real& sn, real& cn, real& dn) const {
  ...
  static const real tolJAC = sqrt(numeric_limits<real>::epsilon() * real(0.01));
  if (_kp2 != 0) {
    real mc = _kp2, d = 0;
    if (_kp2 < 0) {
      d = 1 - mc;
...
Math::real EllipticFunction::E(real sn, real cn, real dn) const {
  real cn2 = cn * cn, dn2 = dn * dn, sn2 = sn * sn,
       ei =
         cn2 != 0
           ? abs(sn) *
               (_k2 <= 0 ?
...
Math::real EllipticFunction::H(real sn, real cn, real dn) const {
  real cn2 = cn * cn, dn2 = dn * dn, sn2 = sn * sn,
    ...
    hi =
      cn2 != 0
        ? abs(sn) * (RF(cn2, dn2, 1) -
                     _alphap2 * sn2 * RJ(cn2, dn2, 1, cn2 + _alphap2 * sn2) / 3)
        : H();
  // Enforce usual trig-like symmetries
  if (cn < 0) hi = 2 * H() - hi;
