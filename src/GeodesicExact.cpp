...
Math::real GeodesicExact::GenDirect(real lat1, real lon1, real azi1,
    ...
    return GeodesicLineExact(*this, lat1, lon1, azi1, outmask)
        .  // Note the dot!
        GenPosition(arcmode, s12_a12, outmask, lat2, lon2, azi2, s12, m12, M12,
                    M21, S12);
}
...
Math::real GeodesicExact::GenInverse(real lat1, real lon1, real lat2, real lon2,
    ...
    if (meridian) {
        ...
        {
            real dummy;
            Lengths(E, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1,
                    cbet2, outmask | REDUCEDLENGTH, s12x, m12x, dummy, M12,
                    M21);
        }
    ...
    if (!meridian && sbet1 == 0 &&  // and sbet2 == 0
        ...
        real dnm;
        sig12 = InverseStart(E, sbet1, cbet1, dn1, sbet2, cbet2, dn2, lam12,
                             slam12, clam12, salp1, calp1, salp2, calp2, dnm);
        ...
        if (sig12 >= 0) {
            ...
            for (bool tripn = false, tripb = false;
                ...
                if (v > 0 &&
                    (numit > maxit1_ || calp1 / salp1 > calp1b / salp1b)) {
                    salp1b = salp1;
                    calp1b = calp1;
                } else if (v < 0 && (numit > maxit1_ ||
            ...
            }
            {
                real dummy;
                Lengths(E, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1,
                        cbet2, outmask, s12x, m12x, dummy, M12, M21);
            }
            m12x *= _b;
            s12x *= _b;
            a12 = sig12 / Math::degree();
...
Math::real GeodesicExact::GenInverse(real lat1, real lon1, real lat2, real lon2,
    ...
    real salp1, calp1, salp2, calp2,
        a12 = GenInverse(lat1, lon1, lat2, lon2, outmask, s12, salp1, calp1,
...
void GeodesicExact::Lengths(const EllipticFunction& E, real sig12, real ssig1,
    ...
    if (outmask & DISTANCE)
        ...
        s12b = E.E() / (Math::pi() / 2) *
               (sig12 +
                (E.deltaE(ssig2, csig2, dn2) - E.deltaE(ssig1, csig1, dn1)));
...
Math::real GeodesicExact::InverseStart(EllipticFunction& E, real sbet1,
    ...
    if (shortline && ssig12 < _etol2) {
        ...
        calp2 = sbet12 - cbet1 * sbet2 *
                             (comg12 >= 0 ? Math::sq(somg12) / (1 + comg12)
                                          : 1 - comg12);
        ...
        if (_f >= 0) {  // In fact f == 0 does not get here
            ...
            // Inverse.
            Lengths(E, Math::pi() + bet12a, sbet1, -cbet1, dn1, sbet2, cbet2,
                    dn2, cbet1, cbet2, REDUCEDLENGTH, dummy, m12b, m0, dummy,
                    dummy);
...
Math::real GeodesicExact::Lambda12(real sbet1, real cbet1, real dn1, real sbet2,
    ...
    if (diffp) {
        if (calp2 == 0)
            ...
            real dummy;
            Lengths(E, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1,
                    cbet2, REDUCEDLENGTH, dummy, dlam12, dummy, dummy, dummy);
