...
const GeodesicExact& GeodesicExact::WGS84() {
  static const GeodesicExact wgs84(Constants::WGS84_a(), Constants::WGS84_f());
  return wgs84;
}

Math::real GeodesicExact::CosSeries(real sinx, real cosx, const real c[],
                                    int n) {
  // Evaluate
  ...
  real ar = 2 * (cosx - sinx) * (cosx + sinx),  // 2 * cos(2 * x)
...
Math::real GeodesicExact::GenDirect(real lat1, real lon1, real azi1,
                                    ...
                                    unsigned outmask, real& lat2, real& lon2,
                                    real& azi2, real& s12, real& m12, real& M12,
                                    real& M21, real& S12) const {
  // Automatically supply DISTANCE_IN if necessary
  if (!arcmode) outmask |= DISTANCE_IN;
  return GeodesicLineExact(*this, lat1, lon1, azi1, outmask)
    .  // Note the dot!
    GenPosition(arcmode, s12_a12, outmask, lat2, lon2, azi2, s12, m12, M12, M21,
                S12);
}

GeodesicLineExact GeodesicExact::GenDirectLine(real lat1, real lon1, real azi1,
                                               bool arcmode, real s12_a12,
                                               unsigned caps) const {
  ...
  return GeodesicLineExact(*this, lat1, lon1, azi1, salp1, calp1, caps, arcmode,
                           s12_a12);
}

GeodesicLineExact GeodesicExact::DirectLine(real lat1, real lon1, real azi1,
                                            real s12, unsigned caps) const {
  return GenDirectLine(lat1, lon1, azi1, false, s12, caps);
}

GeodesicLineExact GeodesicExact::ArcDirectLine(real lat1, real lon1, real azi1,
                                               real a12, unsigned caps) const {
  return GenDirectLine(lat1, lon1, azi1, true, a12, caps);
}

Math::real GeodesicExact::GenInverse(real lat1, real lon1, real lat2, real lon2,
                                     unsigned outmask, real& s12, real& salp1,
                                     real& calp1, real& salp2, real& calp2,
  ...
  real lam12 = lon12 * Math::degree(), slam12, clam12;
  if (lon12 > 90) {
  ...
  real dn1 = (_f >= 0 ? sqrt(1 + _ep2 * Math::sq(sbet1))
  ...
  if (meridian) {
    ...
    {
      real dummy;
      Lengths(E, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2,
              outmask | REDUCEDLENGTH, s12x, m12x, dummy, M12, M21);
    }
    ...
    if (sig12 < 1 || m12x >= 0) {
      ...
      if (sig12 < 3 * tiny_) sig12 = m12x = s12x = 0;
      m12x *= _b;
      s12x *= _b;
  ...
  if (!meridian && sbet1 == 0 &&  // and sbet2 == 0
    ...
    m12x = _b * sin(sig12);
    if (outmask & GEODESICSCALE) M12 = M21 = cos(sig12);
    a12 = lon12 / _f1;
    ...
    real dnm;
    sig12 = InverseStart(E, sbet1, cbet1, dn1, sbet2, cbet2, dn2, lam12, slam12,
                         clam12, salp1, calp1, salp2, calp2, dnm);

    if (sig12 >= 0) {
      // Short lines (InverseStart sets salp2, calp2, dnm)
      s12x = sig12 * _b * dnm;
      m12x = Math::sq(dnm) * _b * sin(sig12 / dnm);
      if (outmask & GEODESICSCALE) M12 = M21 = cos(sig12 / dnm);
      a12 = sig12 / Math::degree();
      omg12 = lam12 / (_f1 * dnm);
    } else {
      // Newton's method.  This is a straightforward solution of f(alp1) =
      // lambda12(alp1) - lam12 = 0 with one wrinkle.  f(alp) has exactly
      // one root in the interval (0, pi) and its derivative is positive
      // at the root.  Thus f(alp) is positive for alp > alp1 and negative
      // for alp < alp1.  During the course of the iteration, a range
      // (alp1a, alp1b) is maintained which brackets the root and with
      // each evaluation of f(alp) the range is shrunk, if possible.
      // Newton's method is restarted whenever the derivative of f is
      // negative (because the new value of alp1 is then further from the
      // solution) or if the new estimate of alp1 lies outside (0,pi); in
      // this case, the new starting guess is taken to be (alp1a + alp1b)
      // / 2.
      //
      ...
      for (bool tripn = false, tripb = false;
           numit < maxit2_ || GEOGRAPHICLIB_PANIC; ++numit) {
        // 1/4 meridian = 10e6 m and random input.  max err is estimated
        // max error in nm (checking solution of inverse problem by
        // direct solution).  iter is mean and sd of number of
        // iterations
        //
        //           max   iter
        ...
        real v = Lambda12(sbet1, cbet1, dn1, sbet2, cbet2, dn2, salp1, calp1,
                          ...
                          ssig2, csig2, E, domg12, numit < maxit1_, dv);
        ...
        // This mechanism is not needed for the WGS84 ellipsoid, but it
        // does catch problems with more eccentric ellipsoids.  Its
        // efficacy is such for the WGS84 test set with the starting
        // guess set to alp1 = 90deg: the WGS84 test set: mean = 5.21,
        // sd = 3.93, max = 24 WGS84 and random input: mean = 4.74, sd =
        // 0.99
      ...
      }
      {
        real dummy;
        Lengths(E, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2,
                outmask, s12x, m12x, dummy, M12, M21);
      }
      m12x *= _b;
      s12x *= _b;
      a12 = sig12 / Math::degree();
  ...
  if (outmask & DISTANCE) s12 = 0 + s12x;  // Convert -0 to 0

  if (outmask & REDUCEDLENGTH) m12 = 0 + m12x;  // Convert -0 to 0
  ...
  if (outmask & AREA) {
    ...
    if (calp0 != 0 && salp0 != 0) {
      real
        ...
        ssig1 = sbet1,
        csig1 = calp1 * cbet1, ssig2 = sbet2, csig2 = calp2 * cbet2,
        k2 = Math::sq(calp0) * _ep2, eps = k2 / (2 * (1 + sqrt(1 + k2)) + k2),
      ...
      real C4a[nC4_];
      C4f(eps, C4a);
      real B41 = CosSeries(ssig1, csig1, C4a, nC4_),
    ...
    if (!meridian &&
      ...
      // viz salp12 = -0 and alp12 = -180.  However this depends on the
      // sign being attached to 0 correctly.  The following ensures the
      // correct behavior.
  ...
  if (swapp < 0) {
    swap(salp1, salp2);
    swap(calp1, calp2);
    if (outmask & GEODESICSCALE) swap(M12, M21);
  }
...
Math::real GeodesicExact::GenInverse(real lat1, real lon1, real lat2, real lon2,
                                     unsigned outmask, real& s12, real& azi1,
                                     real& azi2, real& m12, real& M12,
                                     real& M21, real& S12) const {
  outmask &= OUT_MASK;
  real salp1, calp1, salp2, calp2,
    a12 = GenInverse(lat1, lon1, lat2, lon2, outmask, s12, salp1, calp1, salp2,
                     calp2, m12, M12, M21, S12);
  if (outmask & AZIMUTH) {
...
GeodesicLineExact GeodesicExact::InverseLine(real lat1, real lon1, real lat2,
                                             real lon2, unsigned caps) const {
  real t, salp1, calp1, salp2, calp2,
    a12 = GenInverse(lat1, lon1, lat2, lon2,
                     // No need to specify AZIMUTH here
                     0u, t, salp1, calp1, salp2, calp2, t, t, t, t),
    azi1 = Math::atan2d(salp1, calp1);
  // Ensure that a12 can be converted to a distance
  if (caps & (OUT_MASK & DISTANCE_IN)) caps |= DISTANCE;
  return GeodesicLineExact(*this, lat1, lon1, azi1, salp1, calp1, caps, true,
                           a12);
}

void GeodesicExact::Lengths(const EllipticFunction& E, real sig12, real ssig1,
                            real csig1, real dn1, real ssig2, real csig2,
                            real dn2, real cbet1, real cbet2, unsigned outmask,
                            real& s12b, real& m12b, real& m0, real& M12,
                            real& M21) const {
  ...
  if (outmask & (REDUCEDLENGTH | GEODESICSCALE)) {
    real m0x = -E.k2() * E.D() / (Math::pi() / 2),
         J12 = m0x * (sig12 + (E.deltaD(ssig2, csig2, dn2) -
...
Math::real GeodesicExact::Astroid(real x, real y) {
  ...
  real k;
  real p = Math::sq(x), q = Math::sq(y), r = (p + q - 1) / 6;
  if (!(q == 0 && r <= 0)) {
    real
      // Avoid possible division by zero when r = 0 by multiplying
      // equations for s and t by r^3 and r, resp.
      S = p * q / 4,  // S = r^3 * s
      r2 = Math::sq(r), r3 = r * r2,
    ...
    real v = sqrt(Math::sq(u) + q),      // guaranteed positive
...
Math::real GeodesicExact::InverseStart(EllipticFunction& E, real sbet1,
                                       real cbet1, real dn1, real sbet2,
                                       real cbet2, real dn2, real lam12,
                                       real slam12, real clam12, real& salp1,
                                       real& calp1,
  ...
  real sig12 = -1,  // Return value
                    // bet12 = bet2 - bet1 in [0, pi); bet12a = bet2 + bet1 in
                    // (-pi, 0]
  ...
  real ssig12 = Math::hypot(salp1, calp1),
  ...
  if (shortline && ssig12 < _etol2) {
    ...
    if (_f >= 0) {                          // In fact f == 0 does not get here
      ...
      // Inverse.
      Lengths(E, Math::pi() + bet12a, sbet1, -cbet1, dn1, sbet2, cbet2, dn2,
              cbet1, cbet2, REDUCEDLENGTH, dummy, m12b, m0, dummy, dummy);
      x = -1 + m12b / (cbet1 * cbet2 * m0 * Math::pi());
      betscale =
        x < -real(0.01) ? sbet12a / x : -_f * Math::sq(cbet1) * Math::pi();
    ...
    if (y > -tol1_ && x > -1 - xthresh_) {
      ...
      // of Newton iterations for astroid cases from 2.24 (min 0, max 6)
      // to 2.12 (min 0 max 5).  The changes in the number of iterations
      // are as follows:
      //
      // change percent
      //    1       5
      ...
      // estimating alp1 directly, n = number of iterations estimating via
      // omg12, total number of trials = 148605):
      //
      //  iter    m      n
      //    0   148    186
      //    1 13046  13845
      ...
      real k = Astroid(x, y);
      real omg12a = lamscale * (_f >= 0 ? -x * k / (1 + k) : -y * (1 + k) / k);
      somg12 = sin(omg12a);
      comg12 = -cos(omg12a);
...
Math::real GeodesicExact::Lambda12(real sbet1, real cbet1, real dn1, real sbet2,
                                   real cbet2, real dn2, real salp1, real calp1,
                                   real slam120, real clam120, real& salp2,
                                   real& calp2, real& sig12, real& ssig1,
                                   real& csig1, real& ssig2, real& csig2,
                                   EllipticFunction& E, real& domg12,
  ...
  real schi12 = max(real(0), cchi1 * somg2 - somg1 * cchi2),
  ...
  if (diffp) {
    if (calp2 == 0)
      ...
      real dummy;
      Lengths(E, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2,
              REDUCEDLENGTH, dummy, dlam12, dummy, dummy, dummy);
...
void GeodesicExact::C4f(real eps, real c[]) const {
  ...
  if (!(o == nC4x_)) throw GeographicErr("C4 misalignment");
}
