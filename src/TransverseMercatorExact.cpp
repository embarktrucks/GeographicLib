...
TransverseMercatorExact::TransverseMercatorExact(real a, real f, real k0,
  ...
  if (!(_f > 0)) throw GeographicErr("Flattening is not positive");
  if (!(_f < 1)) throw GeographicErr("Polar semi-axis is not positive");
...
const TransverseMercatorExact& TransverseMercatorExact::UTM() {
  static const TransverseMercatorExact utm(
    Constants::WGS84_a(), Constants::WGS84_f(), Constants::UTM_k0());
  return utm;
}
...
void TransverseMercatorExact::zeta(real /*u*/, real snu, real cnu, real dnu,
  ...
  real d1 = sqrt(Math::sq(cnu) + _mv * Math::sq(snu * snv)),
...
bool TransverseMercatorExact::zetainv0(real psi, real lam, real& u,
                                       real& v) const {
  bool retval = false;
  if (psi < -_e * Math::pi() / 4 && lam > (1 - 2 * _e) * Math::pi() / 2 &&
    ...
    real psix = 1 - psi / _e, lamx = (Math::pi() / 2 - lam) / _e;
    u = Math::asinh(sin(lamx) / Math::hypot(cos(lamx), sinh(psix))) *
        (1 + _mu / 2);
    v = atan2(cos(lamx), sinh(psix)) * (1 + _mu / 2);
    u = _Eu.K() - u;
    v = _Ev.K() - v;
  } else if (psi < _e * Math::pi() / 2 && lam > (1 - 2 * _e) * Math::pi() / 2) {
    ...
    real dlam = lam - (1 - _e) * Math::pi() / 2, rad = Math::hypot(psi, dlam),
         // atan2(dlam-psi, psi+dlam) + 45d gives arg(zeta - zeta0) in range
         // [-135, 225).  Subtracting 180 (since multiplier is negative)
         // makes range [-315, 45).  Multiplying by 1/3 (for cube root)
         // gives range
         // [-105, 15).  In particular the range [-90, 180] in zeta space
         // maps to [-90, 0] in w space as required.
...
void TransverseMercatorExact::zetainv(real taup, real lam, real& u,
                                      real& v) const {
  real psi = Math::asinh(taup), scal = 1 / Math::hypot(real(1), taup);
  if (zetainv0(psi, lam, u, v)) return;
  ...
  for (int i = 0, trip = 0; i < numit_ || GEOGRAPHICLIB_PANIC; ++i) {
    ...
    lam1 -= lam;
    tau1 *= scal;
    real delu = tau1 * du1 - lam1 * dv1, delv = tau1 * dv1 + lam1 * du1;
    u -= delu;
    v -= delv;
    if (trip) break;
...
void TransverseMercatorExact::dwdsigma(real /*u*/, real snu, real cnu, real dnu,
  ...
  // A+S 16.21.4
  real d = _mv * Math::sq(Math::sq(cnv) + _mu * Math::sq(snu * snv));
  real dnr = dnu * cnv * dnv, dni = -_mu * snu * cnu * snv;
  du = (Math::sq(dnr) - Math::sq(dni)) / d;
  dv = 2 * dnr * dni / d;
}

// Starting point for sigmainv
bool TransverseMercatorExact::sigmainv0(real xi, real eta, real& u,
                                        real& v) const {
  bool retval = false;
  if (eta > real(1.25) * _Ev.KE() ||
    ...
    real x = xi - _Eu.E(), y = eta - _Ev.KE(), r2 = Math::sq(x) + Math::sq(y);
    u = _Eu.K() + x / r2;
    v = _Ev.K() - y / r2;
    ...
    real deta = eta - _Ev.KE(), rad = Math::hypot(xi, deta),
         // Map the range [-90, 180] in sigma space to [-90, 0] in w space.
         // See discussion in zetainv0 on the cut for ang.
    ...
    // Else use w = sigma * Eu.K/Eu.E (which is correct in the limit _e ->
    // 0)
    u = xi * _Eu.K() / _Eu.E();
    v = eta * _Eu.K() / _Eu.E();
  }
...
void TransverseMercatorExact::sigmainv(real xi, real eta, real& u,
                                       real& v) const {
  if (sigmainv0(xi, eta, u, v)) return;
  ...
  for (int i = 0, trip = 0; i < numit_ || GEOGRAPHICLIB_PANIC; ++i) {
    ...
    xi1 -= xi;
    eta1 -= eta;
    real delu = xi1 * du1 - eta1 * dv1, delv = xi1 * dv1 + eta1 * du1;
    u -= delu;
    v -= delv;
    if (trip) break;
    real delw2 = Math::sq(delu) + Math::sq(delv);
...
void TransverseMercatorExact::Scale(real tau, real /*lam*/, real snu, real cnu,
                                    real dnu, real snv, real cnv, real dnv,
...
void TransverseMercatorExact::Forward(real lon0, real lat, real lon, real& x,
                                      real& y, real& gamma, real& k) const {
  lat = Math::LatFix(lat);
  lon = Math::AngDiff(lon0, lon);
  // Explicitly enforce the parity
  int latsign = (!_extendp && lat < 0) ? -1 : 1,
  ...
  real lam = lon * Math::degree(), tau = Math::tand(lat);
  ...
  if (backside) gamma = 180 - gamma;
  gamma *= latsign * lonsign;
  k *= _k0;
}

void TransverseMercatorExact::Reverse(real lon0, real x, real y, real& lat,
                                      real& lon, real& gamma, real& k) const {
  // This undoes the steps in Forward.
  real xi = y / (_a * _k0), eta = x / (_a * _k0);
  // Explicitly enforce the parity
  int latsign = !_extendp && y < 0 ? -1 : 1,
