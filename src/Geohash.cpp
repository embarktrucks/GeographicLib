...
void Geohash::Forward(real lat, real lon, int len, std::string& geohash) {
  ...
  unsigned long long ulon = (unsigned long long)(floor(lon / loneps) + shift),
...
void Geohash::Reverse(const std::string& geohash, real& lat, real& lon,
  ...
  if (len1 >= 3 && ((toupper(geohash[0]) == 'I' && toupper(geohash[1]) == 'N' &&
                     toupper(geohash[2]) == 'V') ||
                    // Check A first because it is not in a standard geohash
                    (toupper(geohash[1]) == 'A' && toupper(geohash[0]) == 'N' &&
                     toupper(geohash[2]) == 'N'))) {
    lat = lon = Math::NaN();
    return;
  }
