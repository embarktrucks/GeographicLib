...
const char* const MGRS::upscols_[] = {"JKLPQRSTUXYZ", "ABCFGHJKLPQR", "RSTUXYZ",
                                      "ABCFGHJ"};
const char* const MGRS::upsrows_[] = {"ABCDEFGHJKLMNPQRSTUVWXYZ",
                                      "ABCDEFGHJKLMNP"};
...
const int MGRS::mineasting_[] = {minupsSind_, minupsNind_, minutmcol_,
                                 minutmcol_};
const int MGRS::maxeasting_[] = {maxupsSind_, maxupsNind_, maxutmcol_,
                                 maxutmcol_};
const int MGRS::minnorthing_[] = {minupsSind_, minupsNind_, minutmSrow_,
                                  minutmSrow_ - (maxutmSrow_ - minutmNrow_)};
const int MGRS::maxnorthing_[] = {maxupsSind_, maxupsNind_,
                                  maxutmNrow_ + (maxutmSrow_ - minutmNrow_),
                                  maxutmNrow_};

void MGRS::Forward(int zone, bool northp, real x, real y, real lat, int prec,
                   std::string& mgrs) {
    // The smallest angle s.t., 90 - angeps() < 90 (approx 50e-12 arcsec)
    // 7 = ceil(log_2(90))
    static const real angeps = ldexp(real(1), -(Math::digits() - 7));
    if (zone == UTMUPS::INVALID || Math::isnan(x) || Math::isnan(y) ||
        Math::isnan(lat)) {
        mgrs = "INVALID";
        return;
    }
    ...
    if (!(prec >= -1 && prec <= maxprec_))
        throw GeographicErr("MGRS precision " + Utility::str(prec) +
                            " not in [-1, " + Utility::str(int(maxprec_)) +
                            "]");
    ...
    if (utmp) {
        ...
        mgrs1[z++] =
            utmrow_[(yh + (zone1 & 1 ? utmevenrowshift_ : 0)) % utmrowperiod_];
    } else {
        ...
        mgrs1[z++] =
            upscols_[iband]
                    [xh - (eastp ? upseasting_
                                 : (northp ? minupsNind_ : minupsSind_))];
        mgrs1[z++] =
            upsrows_[northp][yh - (northp ? minupsNind_ : minupsSind_)];
    }
    if (prec > 0) {
        ix -= m * xh;
        iy -= m * yh;
...
void MGRS::Forward(int zone, bool northp, real x, real y, int prec,
                   std::string& mgrs) {
    real lat, lon;
    if (zone > 0) {
        ...
        if (abs(ys) < 1)
            ...
            if (LatitudeBand(latp) == LatitudeBand(late))
                ...
                // bounds straddle a band boundary so need to compute lat
                // accurately
...
void MGRS::Reverse(const std::string& mgrs, int& zone, bool& northp, real& x,
                   real& y, int& prec, bool centerp) {
    int p = 0, len = int(mgrs.length());
    if (len >= 3 && toupper(mgrs[0]) == 'I' && toupper(mgrs[1]) == 'N' &&
        toupper(mgrs[2]) == 'V') {
        zone = UTMUPS::INVALID;
        northp = false;
        x = y = Math::NaN();
    ...
    if (p > 2)
        throw GeographicErr("More than 2 digits at start of MGRS " +
                            mgrs.substr(0, p));
    if (len - p < 1) throw GeographicErr("MGRS string too short " + mgrs);
    bool utmp = zone1 != UTMUPS::UPS;
    int zonem1 = zone1 - 1;
    const char* band = utmp ? latband_ : upsband_;
    int iband = Utility::lookup(band, mgrs[p++]);
    if (iband < 0)
        throw GeographicErr("Band letter " + Utility::str(mgrs[p - 1]) +
                            " not in " + (utmp ? "UTM" : "UPS") + " set " +
                            band);
    ...
    if (irow < 0)
        throw GeographicErr(
            "Row letter " + Utility::str(mgrs[p - 1]) + " not in " +
            (utmp ? "UTM" : "UPS " + Utility::str(hemispheres_[northp1])) +
            " set " + row);
    if (utmp) {
        if (zonem1 & 1)
    ...
    for (int i = 0; i < prec1; ++i) {
        unit *= base_;
        int ix = Utility::lookup(digits_, mgrs[p + i]),
...
void MGRS::CheckCoords(bool utmp, bool& northp, real& x, real& y) {
    ...
    int ix = int(floor(x / tile_)), iy = int(floor(y / tile_)),
    ...
    if (!(ix >= mineasting_[ind] && ix < maxeasting_[ind])) {
        if (ix == maxeasting_[ind] && x == maxeasting_[ind] * tile_)
            ...
            throw GeographicErr(
                "Easting " + Utility::str(int(floor(x / 1000))) +
                "km not in MGRS/" + (utmp ? "UTM" : "UPS") + " range for " +
                (northp ? "N" : "S") + " hemisphere [" +
                Utility::str(mineasting_[ind] * tile_ / 1000) + "km, " +
                Utility::str(maxeasting_[ind] * tile_ / 1000) + "km)");
    }
    if (!(iy >= minnorthing_[ind] && iy < maxnorthing_[ind])) {
        if (iy == maxnorthing_[ind] && y == maxnorthing_[ind] * tile_)
            ...
            throw GeographicErr(
                "Northing " + Utility::str(int(floor(y / 1000))) +
                "km not in MGRS/" + (utmp ? "UTM" : "UPS") + " range for " +
                (northp ? "N" : "S") + " hemisphere [" +
...
int MGRS::UTMRow(int iband, int icol, int irow) {
    ...
    //     9     80     94
    int minrow =
            iband > -10 ? int(floor(c - real(4.3) - real(0.1) * northp)) : -90,
        maxrow =
            iband < 9 ? int(floor(c + real(4.4) - real(0.1) * northp)) : 94,
...
void MGRS::Check() {
    ...
    const short tab[] = {
        ...
        2, 5, 18, 2, 9, 18,  // etc.
        2, 5, 26, 2, 9, 26, 3, 5, 27, 3, 9, 27, 3, 5, 35, 3, 9, 35, 4, 5, 36,
        4, 9, 36, 4, 5, 44, 4, 9, 44, 5, 5, 45, 5, 9, 45, 5, 5, 53, 5, 9, 53,
        6, 5, 54, 6, 9, 54, 6, 5, 62, 6, 9, 62, 7, 5, 63, 7, 9, 63, 7, 5, 70,
        7, 7, 70, 7, 7, 71, 7, 9, 71,            // y = 71t crosses boundary
    ...
    for (int i = 0; i < bandchecks; ++i) {
        UTMUPS::Reverse(38, true, tab[3 * i + 1] * t, tab[3 * i + 2] * t, lat,
                        lon);
        if (!(LatitudeBand(lat) == tab[3 * i + 0]))
            throw GeographicErr(
                "MGRS::Check: Band error, b = " + Utility::str(tab[3 * i + 0]) +
                ", x = " + Utility::str(tab[3 * i + 1]) +
                "00km, y = " + Utility::str(tab[3 * i + 2]) + "00km");
    }
}
