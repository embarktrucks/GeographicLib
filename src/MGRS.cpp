...
void MGRS::Forward(int zone, bool northp, real x, real y, real lat, int prec,
  ...
  if (utmp) {
    ...
    mgrs1[z++] =
      upscols_[iband][xh - (eastp ? upseasting_
...
void MGRS::Reverse(const std::string& mgrs, int& zone, bool& northp, real& x,
  ...
  if (p == len) {  // Grid zone only (ignore centerp)
    ...
    if (utmp) {
      ...
      x = ((iband & 1 ? 1 : -1) * floor(4 * deg + real(0.5)) + upseasting_) *
          tile_;
  ...
  if (irow < 0)
    throw GeographicErr(
      ...
      (utmp ? "UTM" : "UPS " + Utility::str(hemispheres_[northp1])) + " set " +
      row);
  if (utmp) {
    if (zonem1 & 1)
  ...
  if (!(ix >= mineasting_[ind] && ix < maxeasting_[ind])) {
    if (ix == maxeasting_[ind] && x == maxeasting_[ind] * tile_)
      ...
      throw GeographicErr(
        ...
        (utmp ? "UTM" : "UPS") + " range for " + (northp ? "N" : "S") +
...
void MGRS::Check() {
  ...
  if (!(lon < 0)) throw GeographicErr("MGRS::Check: equator coverage failure");
