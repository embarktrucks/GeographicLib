...
int UTMUPS::StandardZone(real lat, real lon, int setzone) {
    ...
    if (setzone >= MINZONE || setzone == INVALID) return setzone;
...
void UTMUPS::Forward(real lat, real lon, int& zone, bool& northp, real& x,
                     real& y, real& gamma, real& k, int setzone,
                     bool mgrslimits) {
    if (abs(lat) > 90)
    ...
    if (utmp) {
        ...
        if (!(dlon <= 60))
            // Check isn't really necessary because CheckCoords catches this
            // case. But this allows a more meaningful error message to be
            // given.
        ...
        if (abs(lat) < 70)
            ...
            throw GeographicErr("Latitude " + Utility::str(lat) +
                                ...
                                (northp1 ? "N" : "S") + " pole");
        PolarStereographic::UPS().Forward(northp1, lat, lon, x1, y1, gamma1,
                                          k1);
    }
...
void UTMUPS::Reverse(int zone, bool northp, real x, real y, real& lat,
                     real& lon, real& gamma, real& k, bool mgrslimits) {
    ...
    if (utmp)
        TransverseMercator::UTM().Reverse(CentralMeridian(zone), x, y, lat, lon,
                                          gamma, k);
    else
...
bool UTMUPS::CheckCoords(bool utmp, bool northp, real x, real y,
    ...
    if (x < mineasting_[ind] - slop || x > maxeasting_[ind] + slop) {
        ...
        throw GeographicErr(
            "Easting " + Utility::str(x / 1000) + "km not in " +
            (mgrslimits ? "MGRS/" : "") + (utmp ? "UTM" : "UPS") +
            " range for " + (northp ? "N" : "S") + " hemisphere [" +
            Utility::str((mineasting_[ind] - slop) / 1000) + "km, " +
            Utility::str((maxeasting_[ind] + slop) / 1000) + "km]");
    }
    if (y < minnorthing_[ind] - slop || y > maxnorthing_[ind] + slop) {
        if (!throwp) return false;
        throw GeographicErr(
            "Northing " + Utility::str(y / 1000) + "km not in " +
            (mgrslimits ? "MGRS/" : "") + (utmp ? "UTM" : "UPS") +
            " range for " + (northp ? "N" : "S") + " hemisphere [" +
            Utility::str((minnorthing_[ind] - slop) / 1000) + "km, " +
            Utility::str((maxnorthing_[ind] + slop) / 1000) + "km]");
    }
    return true;
}
...
void UTMUPS::Transfer(int zonein, bool northpin, real xin, real yin,
    ...
    if (zonein != zoneout) {
        ...
        real x, y;
        int zone1;
        geographic_lib::UTMUPS::Forward(
            lat, lon, zone1, northp, x, y,
            zoneout == UTMUPS::MATCH ? zonein : zoneout);
...
void UTMUPS::DecodeZone(const std::string& zonestr, int& zone, bool& northp) {
    ...
    if (zlen == 0) throw GeographicErr("Empty zone specification");
...
std::string UTMUPS::EncodeZone(int zone, bool northp, bool abbrev) {
    if (zone == INVALID) return string(abbrev ? "inv" : "invalid");
    if (!(zone >= MINZONE && zone <= MAXZONE))
        throw GeographicErr("Zone " + Utility::str(zone) +
                            " not in range [0, 60]");
    ostringstream os;
    if (zone != UPS) os << setfill('0') << setw(2) << zone;
    if (abbrev)
        os << (northp ? 'n' : 's');
    else
...
int UTMUPS::EncodeEPSG(int zone, bool northp) {
    ...
    if (epsg >= 0 && northp) epsg += epsgN - epsgS;
    return epsg;
}
