...
void Geocentric::IntForward(real lat, real lon, real h, real& X, real& Y,
                            real& Z, real M[dim2_]) const {
...
void Geocentric::IntReverse(real X, real Y, real Z, real& lat, real& lon,
                            real& h, real M[dim2_]) const {
    ...
    if (h > _maxrad) {
        ...
        if (!(_e4a * q == 0 && r <= 0)) {
            ...
            if (disc >= 0) {
                ...
                // which avoids cancellation.  Note that disc < 0 implies that r
                // < 0.
                u += 2 * r * cos(ang / 3);
            }
            real v = sqrt(
            ...
            // by 0 in formula for h.  So handle this case by taking the limits:
