...
void Georef::Forward(real lat, real lon, int prec, std::string& georef) {
  ...
  long long x = (long long)(floor(lon * real(m))) - lonorig_ * m,
...
void Georef::Reverse(const std::string& georef, real& lat, real& lon, int& prec,
                     bool centerp) {
  int len = int(georef.length());
  if (len >= 3 && toupper(georef[0]) == 'I' && toupper(georef[1]) == 'N' &&
      toupper(georef[2]) == 'V') {
    lat = lon = Math::NaN();
    return;
  }
  if (len < baselen_ - 2)
    throw GeographicErr("Georef must start with at least 2 letters " + georef);
  ...
  if (len > 2) {
    ...
    lon1 = lon1 * tile_ + k;
    if (len < 4)
      throw GeographicErr("Missing latitude degree letter in georef " + georef);
    ...
    if (prec1 > 0) {
      ...
      for (int i = 0; i < prec1; ++i) {
        int m = i ? base_ : 6;
        unit *= m;
        int x = Utility::lookup(digits_, georef[baselen_ + i]),
