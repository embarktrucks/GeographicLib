...
Math::real GravityCircle::Gravity(real lon, real& gx, real& gy,
                                  real& gz) const {
...
Math::real GravityCircle::GeoidHeight(real lon) const {
    if ((_caps & GEOID_HEIGHT) != GEOID_HEIGHT) return Math::NaN();
...
void GravityCircle::SphericalAnomaly(real lon, real& Dg01, real& xi,
                                     real& eta) const {
...
Math::real GravityCircle::W(real slam, real clam, real& gX, real& gY,
                            real& gZ) const {
...
Math::real GravityCircle::V(real slam, real clam, real& GX, real& GY,
                            real& GZ) const {
    if ((_caps & GRAVITY) != GRAVITY) {
        GX = GY = GZ = Math::NaN();
        return Math::NaN();
    }
    real Vres = _gravitational(slam, clam, GX, GY, GZ), f = _GMmodel / _amodel;
    Vres *= f;
    GX *= f;
    GY *= f;
    GZ *= f;
...
Math::real GravityCircle::InternalT(real slam, real clam, real& deltaX,
    ...
    if (_dzonal0 == 0) correct = false;
    real T = (gradp ? _disturbing(slam, clam, deltaX, deltaY, deltaZ)
