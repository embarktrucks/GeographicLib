 ...
 * \brief Implementation for geographic_lib::GeodesicLine class
 *
...
void GeodesicLine::LineInit(const Geodesic& g, real lat1, real lon1, real azi1,
                            real salp1, real calp1, unsigned caps) {
  tiny_ = g.tiny_;
  ...
  real cbet1, sbet1;
  Math::sincosd(Math::AngRound(_lat1), sbet1, cbet1);
  sbet1 *= _f1;
  ...
  if (_caps & CAP_C1p) Geodesic::C1pf(eps, _C1pa);
...
GeodesicLine::GeodesicLine(const Geodesic& g, real lat1, real lon1, real azi1,
                           unsigned caps) {
...
GeodesicLine::GeodesicLine(const Geodesic& g, real lat1, real lon1, real azi1,
                           real salp1, real calp1, unsigned caps, bool arcmode,
                           real s13_a13) {
  LineInit(g, lat1, lon1, azi1, salp1, calp1, caps);
...
Math::real GeodesicLine::GenPosition(bool arcmode, real s12_a12,
  ...
  if (arcmode) {
    ...
    real tau12 = s12_a12 / (_b * (1 + _A1m1)), s = sin(tau12), c = cos(tau12);
    // tau2 = tau1 + tau12
    B12 = -Geodesic::SinCosSeries(true, _stau1 * c + _ctau1 * s,
                                  _ctau1 * c - _stau1 * s, _C1pa, nC1p_);
    ...
    if (abs(_f) > 0.01) {
      ...
      real ssig2 = _ssig1 * csig12 + _csig1 * ssig12,
  ...
  if (outmask & LONGITUDE) {
    ...
    real omg12 =
      outmask & LONG_UNROLL
        ? E * (sig12 - (atan2(ssig2, csig2) - atan2(_ssig1, _csig1)) +
    ...
    real lam12 =
      ...
      _A3c *
        (sig12 +
         (Geodesic::SinCosSeries(true, ssig2, csig2, _C3a, nC3_ - 1) - _B31));
    real lon12 = lam12 / Math::degree();
    lon2 = outmask & LONG_UNROLL
             ? _lon1 + lon12
             : Math::AngNormalize(Math::AngNormalize(_lon1) +
                                  Math::AngNormalize(lon12));
  }

  if (outmask & LATITUDE) lat2 = Math::atan2d(sbet2, _f1 * cbet2);

  if (outmask & AZIMUTH) azi2 = Math::atan2d(salp2, calp2);

  if (outmask & (REDUCEDLENGTH | GEODESICSCALE)) {
    real B22 = Geodesic::SinCosSeries(true, ssig2, csig2, _C2a, nC2_),
  ...
  if (outmask & AREA) {
    real B42 = Geodesic::SinCosSeries(false, ssig2, csig2, _C4a, nC4_);
    real salp12, calp12;
    if (_calp0 == 0 || _salp0 == 0) {
      ...
      // out to be wrong once _salp1 = -0 was allowed (via
