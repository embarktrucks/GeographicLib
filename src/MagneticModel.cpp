...
MagneticModel::MagneticModel(const std::string& name, const std::string& path,
  ...
  : _name(name),
    ...
    _earth(earth) {
  if (_dir.empty()) _dir = DefaultMagneticPath();
  ReadMetadata(_name);
...
void MagneticModel::ReadMetadata(const std::string& name) {
  ...
  if (!(_t0 > 0)) throw GeographicErr("Epoch time not defined");
  if (_tmin >= _tmax) throw GeographicErr("Min time exceeds max time");
  if (_hmin >= _hmax) throw GeographicErr("Min height exceeds max height");
  if (int(_id.size()) != idlength_) throw GeographicErr("Invalid ID");
  if (_Nmodels < 1) throw GeographicErr("NumModels must be positive");
...
void MagneticModel::Field(real t, real lat, real lon, real h, bool diffp,
  ...
  if (_Nconstants) _harm[_Nmodels + 1](X, Y, Z, BXc, BYc, BZc);
  if (interpolate) {
...
MagneticCircle MagneticModel::Circle(real t, real lat, real h) const {
  ...
  return (_Nconstants == 0
            ? MagneticCircle(_a, _earth._f, lat, h, t, M[7], M[8], t1, _dt0,
                             interpolate, _harm[n].Circle(X, Z, true),
...
void MagneticModel::FieldComponents(real Bx, real By, real Bz, real Bxt,
                                    real Byt, real Bzt, real& H, real& F,
                                    real& D, real& I, real& Ht, real& Ft,
                                    real& Dt, real& It) {
  H = Math::hypot(Bx, By);
...
std::string MagneticModel::DefaultMagneticPath() {
  ...
  if (magneticpath) path = string(magneticpath);
...
std::string MagneticModel::DefaultMagneticName() {
  ...
  if (magneticname) name = string(magneticname);
