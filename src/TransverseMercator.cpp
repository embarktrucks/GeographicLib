 ...
 * \brief Implementation for geographic_lib::TransverseMercator class
 *
...
#error "Bad value for GEOGRAPHICLIB_TRANSVERSEMERCATOR_ORDER"
  ...
  GEOGRAPHICLIB_STATIC_ASSERT(sizeof(b1coeff) / sizeof(real) == maxpow_ / 2 + 2,
                              "Coefficient array size mismatch for b1");
  GEOGRAPHICLIB_STATIC_ASSERT(
    sizeof(alpcoeff) / sizeof(real) == (maxpow_ * (maxpow_ + 3)) / 2,
    "Coefficient array size mismatch for alp");
  GEOGRAPHICLIB_STATIC_ASSERT(
    sizeof(betcoeff) / sizeof(real) == (maxpow_ * (maxpow_ + 3)) / 2,
...
void TransverseMercator::Forward(real lon0, real lat, real lon, real& x,
  ...
  //    phi[0](x) = [0; 0]
  //    phi[1](x) = [sin(x); cos(x)]
  real c0 = cos(2 * xip), ch0 = cosh(2 * etap), s0 = sin(2 * xip),
       sh0 = sinh(2 * etap);
...
void TransverseMercator::Reverse(real lon0, real x, real y, real& lat,
  ...
  real c0 = cos(2 * xi), ch0 = cosh(2 * eta), s0 = sin(2 * xi),
       sh0 = sinh(2 * eta);
  complex<real> a(2 * c0 * ch0, -2 * s0 * sh0);  // 2 * cos(2*zeta)
  int n = maxpow_;
