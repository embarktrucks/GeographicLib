...
Rhumb::Rhumb(real a, real f, bool exact)
  : _ell(a, f), _exact(exact), _c2(_ell.Area() / 720) {
...
#error "Bad value for GEOGRAPHICLIB_RHUMBAREA_ORDER"
  ...
  GEOGRAPHICLIB_STATIC_ASSERT(
    sizeof(coeff) / sizeof(real) == ((maxpow_ + 1) * (maxpow_ + 4)) / 2,
  ...
  for (int l = 0; l <= maxpow_; ++l) {
    ...
    if (l) _R[l] = d * Math::polyval(m, coeff + o, _ell._n) / coeff[o + m + 1];
    o += m + 2;
    d *= _ell._n;
  }
...
void Rhumb::GenInverse(real lat1, real lon1, real lat2, real lon2,
                       ...
                       real& S12) const {
  real lon12 = Math::AngDiff(lon1, lon2), psi1 = _ell.IsometricLatitude(lat1),
       psi2 = _ell.IsometricLatitude(lat2), psi12 = psi2 - psi1,
       h = Math::hypot(lon12, psi12);
  if (outmask & AZIMUTH) azi12 = Math::atan2d(lon12, psi12);
  if (outmask & DISTANCE) {
    real dmudpsi = DIsometricToRectifying(psi2, psi1);
    s12 = h * dmudpsi * _ell.QuarterMeridian() / 90;
  }
  if (outmask & AREA)
    S12 = _c2 * lon12 * MeanSinXi(psi2 * Math::degree(), psi1 * Math::degree());
}
...
Math::real Rhumb::DE(real x, real y) const {
  ...
  real d = x - y;
  if (x * y <= 0) return d != 0 ? (ei.E(x) - ei.E(y)) / d : 1;
  ...
  real Dt = Dsin(x, y) * (sx + sy) /
       ...
       t = d * Dt, Dsz = 2 * Dt / (1 + t * t), sz = d * Dsz,
       cz = (1 - t) * (1 + t) / (1 + t * t);
...
Math::real Rhumb::DRectifying(real latx, real laty) const {
  real tbetx = _ell._f1 * Math::tand(latx), tbety = _ell._f1 * Math::tand(laty);
...
Math::real Rhumb::SinCosSeries(bool sinp, real x, real y, const real c[],
  ...
  real p = x + y, d = x - y, cp = cos(p), cd = cos(d), sp = sin(p),
       sd = d != 0 ? sin(d) / d : 1, m = 2 * cp * cd, s = sp * sd;
...
Math::real Rhumb::DConformalToRectifying(real chix, real chiy) const {
  return 1 + SinCosSeries(true, chix, chiy, _ell.ConformalToRectifyingCoeffs(),
                          tm_maxord);
}

Math::real Rhumb::DRectifyingToConformal(real mux, real muy) const {
  return 1 - SinCosSeries(true, mux, muy, _ell.RectifyingToConformalCoeffs(),
                          tm_maxord);
}

Math::real Rhumb::DIsometricToRectifying(real psix, real psiy) const {
  if (_exact) {
    real latx = _ell.InverseIsometricLatitude(psix),
...
Math::real Rhumb::DRectifyingToIsometric(real mux, real muy) const {
  real latx = _ell.InverseRectifyingLatitude(mux / Math::degree()),
       laty = _ell.InverseRectifyingLatitude(muy / Math::degree());
  return _exact ? DIsometric(latx, laty) / DRectifying(latx, laty)
...
void RhumbLine::GenPosition(real s12, unsigned outmask, real& lat2, real& lon2,
                            real& S12) const {
  real mu12 = s12 * _calp * 90 / _rh._ell.QuarterMeridian(), mu2 = _mu1 + mu12;
