...
Math::real Geodesic::SinCosSeries(bool sinp, real sinx, real cosx,
                                  const real c[], int n) {
    // Evaluate
...
Math::real Geodesic::GenDirect(real lat1, real lon1, real azi1, bool arcmode,
                               real s12_a12, unsigned outmask, real& lat2,
                               real& lon2, real& azi2, real& s12, real& m12,
...
GeodesicLine Geodesic::GenDirectLine(real lat1, real lon1, real azi1,
    ...
    return GeodesicLine(*this, lat1, lon1, azi1, salp1, calp1, caps, arcmode,
                        s12_a12);
}
...
GeodesicLine Geodesic::ArcDirectLine(real lat1, real lon1, real azi1, real a12,
                                     unsigned caps) const {
...
Math::real Geodesic::GenInverse(real lat1, real lon1, real lat2, real lon2,
    ...
    real dn1 = sqrt(1 + _ep2 * Math::sq(sbet1)),
    ...
    if (meridian) {
        ...
        {
            real dummy;
            Lengths(_n, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1,
                    cbet2, outmask | DISTANCE | REDUCEDLENGTH, s12x, m12x,
                    dummy, M12, M21, Ca);
        }
    ...
    if (!meridian && sbet1 == 0 &&  // and sbet2 == 0
        ...
        real dnm;
        sig12 =
            InverseStart(sbet1, cbet1, dn1, sbet2, cbet2, dn2, lam12, slam12,
        ...
        if (sig12 >= 0) {
            ...
            for (bool tripn = false, tripb = false;
                ...
                real dv;
                real v = Lambda12(sbet1, cbet1, dn1, sbet2, cbet2, dn2, salp1,
                                  calp1, slam12, clam12, salp2, calp2, sig12,
            ...
            {
                real dummy;
                // Ensure that the reduced length and geodesic scale are
                // computed in a "canonical" way, with the I2 integral.
                unsigned lengthmask =
                    outmask |
                    (outmask & (REDUCEDLENGTH | GEODESICSCALE) ? DISTANCE
                                                               : NONE);
                Lengths(eps, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1,
                        cbet2, lengthmask, s12x, m12x, dummy, M12, M21, Ca);
            }
            m12x *= _b;
            s12x *= _b;
            a12 = sig12 / Math::degree();
    ...
    if (outmask & AREA) {
        ...
        if (calp0 != 0 && salp0 != 0) {
            ...
            C4f(eps, Ca);
            real B41 = SinCosSeries(false, ssig1, csig1, Ca, nC4_),
...
Math::real Geodesic::GenInverse(real lat1, real lon1, real lat2, real lon2,
                                ...
                                real& azi2, real& m12, real& M12, real& M21,
                                real& S12) const {
    outmask &= OUT_MASK;
...
void Geodesic::Lengths(real eps, real sig12, real ssig1, real csig1, real dn1,
                       real ssig2, real csig2, real dn2, real cbet1, real cbet2,
                       unsigned outmask, real& s12b, real& m12b, real& m0,
                       real& M12, real& M21,
    ...
    if (outmask & DISTANCE) {
        ...
        for (int l = 1; l <= nC2_; ++l) Cb[l] = A1 * Ca[l] - A2 * Cb[l];
...
Math::real Geodesic::InverseStart(real sbet1, real cbet1, real dn1, real sbet2,
                                  real cbet2, real dn2, real lam12, real slam12,
                                  real clam12, real& salp1, real& calp1,
    ...
    if (shortline && ssig12 < _etol2) {
        ...
        if (_f >= 0) {  // In fact f == 0 does not get here
            ...
            // Inverse.
            Lengths(_n, Math::pi() + bet12a, sbet1, -cbet1, dn1, sbet2, cbet2,
...
Math::real Geodesic::Lambda12(real sbet1, real cbet1, real dn1, real sbet2,
                              real cbet2, real dn2, real salp1, real calp1,
                              real slam120, real clam120, real& salp2,
                              real& calp2, real& sig12, real& ssig1,
                              real& csig1, real& ssig2, real& csig2, real& eps,
                              real& domg12, bool diffp, real& dlam12,
                              // Scratch area of the right size
                              real Ca[]) const {
    if (sbet1 == 0 && calp1 == 0)
...
void Geodesic::C3coeff() {
    ...
    static const real coeff[] = {
        ...
        // This is a case where a leading 0 term has been inserted to maintain
        // the
...
#endif
    GEOGRAPHICLIB_STATIC_ASSERT(
        sizeof(coeff) / sizeof(real) == (nC4_ * (nC4_ + 1) * (nC4_ + 5)) / 6,
