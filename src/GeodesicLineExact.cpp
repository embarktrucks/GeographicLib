...
void GeodesicLineExact::LineInit(const GeodesicExact& g, real lat1, real lon1,
...
GeodesicLineExact::GeodesicLineExact(const GeodesicExact& g, real lat1,
                                     real lon1, real azi1, unsigned caps) {
...
GeodesicLineExact::GeodesicLineExact(const GeodesicExact& g, real lat1,
                                     real lon1, real azi1, real salp1,
                                     real calp1, unsigned caps, bool arcmode,
                                     real s13_a13) {
  LineInit(g, lat1, lon1, azi1, salp1, calp1, caps);
...
Math::real GeodesicLineExact::GenPosition(bool arcmode, real s12_a12,
                                          unsigned outmask, real& lat2,
                                          real& lon2, real& azi2, real& s12,
  ...
  if (arcmode) {
    ...
    real tau12 = s12_a12 / (_b * _E0), s = sin(tau12), c = cos(tau12);
    // tau2 = tau1 + tau12
  ...
  if (outmask & DISTANCE) s12 = arcmode ? _b * (_E0 * sig12 + AB1) : s12_a12;
  ...
  if (outmask & LONGITUDE) {
    ...
    real lam12 = chi12 - _e2 / _f1 * _salp0 * _H0 *
  ...
  if (outmask & AREA) {
    real B42 = GeodesicExact::CosSeries(ssig2, csig2, _C4a, nC4_);
    real salp12, calp12;
    if (_calp0 == 0 || _salp0 == 0) {
      ...
      // GeodesicExact::InverseLine).  In fact, the calculation of
      // {s,c}alp12 was already correct (following the IEEE rules for
      // handling signed zeros).  So the patch up code was unnecessary (as
      // well as dangerous).
    } else {
