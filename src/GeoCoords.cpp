...
void GeoCoords::Reset(const std::string& s, bool centerp, bool longfirst) {
  ...
  if (sa.size() == 1) {
    ...
    UTMUPS::Reverse(_zone, _northp, _easting, _northing, _lat, _long, _gamma,
                    _k);
  } else if (sa.size() == 2) {
    DMS::DecodeLatLon(sa[0], sa[1], _lat, _long, longfirst);
    _long = Math::AngNormalize(_long);
    UTMUPS::Forward(_lat, _long, _zone, _northp, _easting, _northing, _gamma,
                    _k);
  } else if (sa.size() == 3) {
...
void GeoCoords::UTMUPSString(int zone, bool northp, real easting, real northing,
  ...
  if (Math::isfinite(easting)) {
    ...
    if (prec < 0 && abs(easting / scale) > real(0.5)) os << setw(-prec) << 0;
  } else
    os << " nan";
  if (Math::isfinite(northing)) {
    os << " " << Utility::str(northing / scale, max(0, prec));
    if (prec < 0 && abs(northing / scale) > real(0.5)) os << setw(-prec) << 0;
  } else
    os << " nan";
  utm = os.str();
}
...
string GeoCoords::UTMUPSRepresentation(bool northp, int prec,
  ...
  real e, n;
  int z;
  UTMUPS::Transfer(_zone, _northp, _easting, _northing, _zone, northp, e, n, z);
  string utm;
...
string GeoCoords::AltUTMUPSRepresentation(bool northp, int prec,
  ...
  real e, n;
  int z;
  UTMUPS::Transfer(_alt_zone, _northp, _alt_easting, _alt_northing, _alt_zone,
                   northp, e, n, z);
  string utm;
