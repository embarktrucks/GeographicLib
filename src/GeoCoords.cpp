...
void GeoCoords::Reset(const std::string& s, bool centerp, bool longfirst) {
    ...
    if (sa.size() == 1) {
        ...
        MGRS::Reverse(sa[0], _zone, _northp, _easting, _northing, prec,
                      centerp);
        UTMUPS::Reverse(_zone, _northp, _easting, _northing, _lat, _long,
                        _gamma, _k);
    } else if (sa.size() == 2) {
        DMS::DecodeLatLon(sa[0], sa[1], _lat, _long, longfirst);
        _long = Math::AngNormalize(_long);
        UTMUPS::Forward(_lat, _long, _zone, _northp, _easting, _northing,
                        _gamma, _k);
    } else if (sa.size() == 3) {
...
string GeoCoords::UTMUPSRepresentation(bool northp, int prec,
    ...
    real e, n;
    int z;
    UTMUPS::Transfer(_zone, _northp, _easting, _northing, _zone, northp, e, n,
                     z);
    string utm;
