...
const int Geoid::c3_[stencilsize_ * nterms_] = {
  9,   -18, -88, 0,    96,  90,   0,   0,   -60, -20, -9,  18,  8,   0,   -96,
  30,  0,   0,   60,   -20, 9,    -88, -18, 90,  96,  0,   -20, -60, 0,   0,
  186, -42, -42, -150, -96, -150, 60,  60,  60,  60,  54,  162, -78, 30,  -24,
  -90, -60, 60,  -60,  60,  -9,   -32, 18,  30,  24,  0,   20,  -60, 0,   0,
  -9,  8,   18,  30,   -96, 0,    -20, 60,  0,   0,   54,  -78, 162, -90, -24,
  30,  60,  -60, 60,   -60, -54,  78,  78,  90,  144, 90,  -60, -60, -60, -60,
  9,   -8,  -18, -30,  -24, 0,    20,  60,  0,   0,   -9,  18,  -32, 0,   24,
  30,  0,   0,   -60,  20,  9,    -18, -8,  0,   -24, -30, 0,   0,   60,  20,
};
...
const int Geoid::c3n_[stencilsize_ * nterms_] = {
  0,    0, -131, 0,   138,  144,  0, 0,   -102, -31, 0,   0, 7,   0,   -138,
  42,   0, 0,    102, -31,  62,   0, -31, 0,    0,   -62, 0, 0,   0,   31,
  124,  0, -62,  0,   0,    -124, 0, 0,   0,    62,  124, 0, -62, 0,   0,
  -124, 0, 0,    0,   62,   62,   0, -31, 0,    0,   -62, 0, 0,   0,   31,
  0,    0, 45,   0,   -183, -9,   0, 93,  18,   0,   0,   0, 216, 0,   33,
  87,   0, -93,  12,  -93,  0,    0, 156, 0,    153, 99,  0, -93, -12, -93,
  0,    0, -45,  0,   -3,   9,    0, 93,  -18,  0,   0,   0, -55, 0,   48,
  42,   0, 0,    -84, 31,   0,    0, -7,  0,    -48, -42, 0, 0,   84,  31,
};
...
const int Geoid::c3s_[stencilsize_ * nterms_] = {
  18,   -36, -122, 0,    120,  135, 0,   0,    -84, -31, -18,  36, -2,  0,
  -120, 51,  0,    0,    84,   -31, 36,  -165, -27, 93,  147,  -9, 0,   -93,
  18,   0,   210,  45,   -111, -93, -57, -192, 0,   93,  12,   93, 162, 141,
  -75,  -93, -129, -180, 0,    93,  -12, 93,   -36, -21, 27,   93, 39,  9,
  0,    -93, -18,  0,    0,    0,   62,  0,    0,   31,  0,    0,  0,   -31,
  0,    0,   124,  0,    0,    62,  0,   0,    0,   -62, 0,    0,  124, 0,
  0,    62,  0,    0,    0,    -62, 0,   0,    62,  0,   0,    31, 0,   0,
  0,    -31, -18,  36,   -64,  0,   66,  51,   0,   0,   -102, 31, 18,  -36,
  2,    0,   -66,  -51,  0,    0,   102, 31,
};
    ...
    _threadsafe(false)  // Set after cache is read
{
  ...
  if (_dir.empty()) _dir = DefaultGeoidPath();
  _filename = _dir + "/" + _name + (pixel_size_ != 4 ? ".pgm" : ".pgm4");
  _file.open(_filename.c_str(), ios::binary);
  if (!(_file.good())) throw GeographicErr("File not readable " + _filename);
  string s;
  ...
  while (getline(_file, s)) {
    ...
    if (s[0] == '#') {
      ...
      if (!(is >> commentid >> key) || commentid != "#") continue;
      if (key == "Description" || key == "DateTime") {
        string::size_type p = s.find_first_not_of(" \t", unsigned(is.tellg()));
        if (p != string::npos)
  ...
  if (_scale == 0) throw GeographicErr("Scale not set " + _filename);
  if (_scale < 0) throw GeographicErr("Scale must be positive " + _filename);
...
Math::real Geoid::height(real lat, real lon) const {
  ...
  real fx = lon * _rlonres, fy = -lat * _rlatres;
  int ix = int(floor(fx)), iy = min((_height - 1) / 2 - 1, int(floor(fy)));
  fx -= ix;
  fy -= iy;
  iy += (_height - 1) / 2;
  ...
  if (!_cubic) {
    real a = (1 - fx) * v00 + fx * v01, b = (1 - fx) * v10 + fx * v11,
         c = (1 - fy) * a + fy * b, h = _offset + _scale * c;
    if (!_threadsafe) {
      _ix = ix;
      _iy = iy;
      _v00 = v00;
...
void Geoid::CacheArea(real south, real west, real north, real east) const {
  ...
  if (east <= west) east += 360;  // east - west in (0, 360]
  int iw = int(floor(west * _rlonres)), ie = int(floor(east * _rlonres)),
  ...
  try {
    ...
    for (int iy = min(oysize, _ysize); iy--;) _data[iy].resize(_xsize);
  ...
  try {
    for (int iy = in; iy <= is; ++iy) {
      ...
      if (iy < 0 || iy >= _height) {
        ...
        iw1 += _width / 2;
        if (iw1 >= _width) iw1 -= _width;
      }
      int xs1 = min(_width - iw1, _xsize);
      filepos(iw1, iy1);
      Utility::readarray<pixel_t, pixel_t, true>(_file, &(_data[iy - in][0]),
                                                 xs1);
      if (xs1 < _xsize) {
...
std::string Geoid::DefaultGeoidPath() {
  ...
  if (geoidpath) path = string(geoidpath);
...
std::string Geoid::DefaultGeoidName() {
  ...
  if (geoidname) name = string(geoidname);
