...
Math::real Ellipsoid::QuarterMeridian() const { return _b * _ell.E(); }
...
Math::real Ellipsoid::RectifyingLatitude(real phi) const {
  return abs(phi) == 90 ? phi : 90 * MeridianDistance(phi) / QuarterMeridian();
}

Math::real Ellipsoid::InverseRectifyingLatitude(real mu) const {
  if (abs(mu) == 90) return mu;
...
Math::real Ellipsoid::CircleHeight(real phi) const {
  ...
  // b * sin(beta)
  return _b * tbeta / Math::hypot(real(1), _f1 * Math::tand(Math::LatFix(phi)));
}
...
Math::real Ellipsoid::NormalCurvatureRadius(real phi, real azi) const {
  real calp, salp, v = 1 - _e2 * Math::sq(Math::sind(Math::LatFix(phi)));
