...
GravityModel::GravityModel(const std::string& name, const std::string& path)
  ...
  if (_dir.empty()) _dir = DefaultGravityPath();
  ReadMetadata(_name);
  {
    string coeff = _filename + ".cof";
    ifstream coeffstr(coeff.c_str(), ios::binary);
    if (!coeffstr.good()) throw GeographicErr("Error opening " + coeff);
    char id[idlength_ + 1];
    coeffstr.read(id, idlength_);
    if (!coeffstr.good()) throw GeographicErr("No header in " + coeff);
    id[idlength_] = '\0';
    if (_id != string(id))
    ...
    if (_Cx[0] != 0) throw GeographicErr("A degree 0 term should be zero");
    ...
    if (pos != coeffstr.tellg()) throw GeographicErr("Extra data in " + coeff);
  }
  ...
  for (int n = 2; n <= nmx; n += 2) {
    // Only include as many normal zonal terms as matter.  Figuring the
    // limit in this way works because the coefficients of the normal
    // potential (which is smooth) decay much more rapidly that the
    // corresponding coefficient of the model potential (which is bumpy).
    // Typically this goes out to n = 18.
    mult *= amult;
    real r = _Cx[n],                                     // the model term
  ...
  _disturbing = SphericalHarmonic1(
    _Cx, _Sx, _gravitational.Coefficients().N(), nmx,
    _gravitational.Coefficients().mmx(), _zonal,
    _zonal,  // This is not accessed!
    nmx1, nmx1, 0, _amodel, SphericalHarmonic1::normalization(_norm));
}
...
void GravityModel::ReadMetadata(const std::string& name) {
  ...
  if (!metastr.good()) throw GeographicErr("Cannot open " + _filename);
  string line;
  ...
  while (getline(metastr, line)) {
    if (!Utility::ParseLine(line, key, val)) continue;
    // Process key words
    if (key == "Name")
      _name = val;
  ...
  if (int(_id.size()) != idlength_) throw GeographicErr("Invalid ID");
  if (Math::isfinite(f) && Math::isfinite(J2))
    throw GeographicErr("Cannot specify both f and J2");
  _earth =
    NormalGravity(a, GM, omega, Math::isfinite(f) ? f : J2, Math::isfinite(f));
}

Math::real GravityModel::InternalT(real X, real Y, real Z, real& deltaX,
                                   real& deltaY, real& deltaZ, bool gradp,
                                   bool correct) const {
...
Math::real GravityModel::V(real X, real Y, real Z, real& GX, real& GY,
                           real& GZ) const {
  real Vres = _gravitational(X, Y, Z, GX, GY, GZ), f = _GMmodel / _amodel;
  Vres *= f;
  GX *= f;
  GY *= f;
  GZ *= f;
...
Math::real GravityModel::W(real X, real Y, real Z, real& gX, real& gY,
                           real& gZ) const {
  real fX, fY, Wres = V(X, Y, Z, gX, gY, gZ) + _earth.Phi(X, Y, fX, fY);
  gX += fX;
  gY += fY;
  return Wres;
}

void GravityModel::SphericalAnomaly(real lat, real lon, real h, real& Dg01,
                                    real& xi, real& eta) const {
  real X, Y, Z, M[Geocentric::dim2_];
  _earth.Earth().IntForward(lat, lon, h, X, Y, Z, M);
  real deltax, deltay, deltaz,
    T = InternalT(X, Y, Z, deltax, deltay, deltaz, true, false), clam = M[3],
    slam = -M[0], P = Math::hypot(X, Y), R = Math::hypot(P, Z),
    // psi is geocentric latitude
    cpsi = R != 0 ? P / R : M[7], spsi = R != 0 ? Z / R : M[8];
...
Math::real GravityModel::GeoidHeight(real lat, real lon) const {
  ...
  real gamma0 = _earth.SurfaceGravity(lat), dummy,
...
Math::real GravityModel::Gravity(real lat, real lon, real h, real& gx, real& gy,
                                 real& gz) const {
  real X, Y, Z, M[Geocentric::dim2_];
  _earth.Earth().IntForward(lat, lon, h, X, Y, Z, M);
...
Math::real GravityModel::Disturbance(real lat, real lon, real h, real& deltax,
                                     real& deltay, real& deltaz) const {
  real X, Y, Z, M[Geocentric::dim2_];
  _earth.Earth().IntForward(lat, lon, h, X, Y, Z, M);
...
GravityCircle GravityModel::Circle(real lat, real h, unsigned caps) const {
  ...
  real invR = 1 / Math::hypot(X, Z),
       gamma0 = (caps & CAP_GAMMA0 ? _earth.SurfaceGravity(lat) : Math::NaN()),
       fx, fy, fz, gamma;
  if (caps & CAP_GAMMA) {
  ...
  return GravityCircle(
    GravityCircle::mask(caps), _earth._a, _earth._f, lat, h, Z, X, M[7], M[8],
    _amodel, _GMmodel, _dzonal0, _corrmult, gamma0, gamma, fx,
    caps & CAP_G ? _gravitational.Circle(X, Z, true) : CircularEngine(),
    // N.B. If CAP_DELTA is set then CAP_T should be too.
    caps & CAP_T ? _disturbing.Circle(-1, X, Z, (caps & CAP_DELTA) != 0)
                 : CircularEngine(),
    caps & CAP_C ? _correction.Circle(invR * X, invR * Z, false)
                 : CircularEngine());
}
...
std::string GravityModel::DefaultGravityPath() {
  ...
  if (gravitypath) path = string(gravitypath);
  if (!path.empty()) return path;
  char* datapath = getenv("GEOGRAPHICLIB_DATA");
  if (datapath) path = string(datapath);
...
std::string GravityModel::DefaultGravityName() {
  ...
  if (gravityname) name = string(gravityname);
