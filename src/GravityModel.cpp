...
GravityModel::GravityModel(const std::string& name, const std::string& path)
    ...
    for (int n = 2; n <= nmx; n += 2) {
        ...
        real r = _Cx[n],  // the model term
            s = -mult * _earth.Jn(n) /
                sqrt(real(2 * n + 1)),  // the normal term
...
void GravityModel::ReadMetadata(const std::string& name) {
    ...
    _earth = NormalGravity(a, GM, omega, Math::isfinite(f) ? f : J2,
                           Math::isfinite(f));
}
...
void GravityModel::SphericalAnomaly(real lat, real lon, real h, real& Dg01,
    ...
    real deltax, deltay, deltaz,
        ...
        clam = M[3], slam = -M[0], P = Math::hypot(X, Y), R = Math::hypot(P, Z),
...
GravityCircle GravityModel::Circle(real lat, real h, unsigned caps) const {
    ...
    real invR = 1 / Math::hypot(X, Z),
         gamma0 =
             (caps & CAP_GAMMA0 ? _earth.SurfaceGravity(lat) : Math::NaN()),
         fx, fy, fz, gamma;
    if (caps & CAP_GAMMA) {
    ...
    return GravityCircle(
        GravityCircle::mask(caps), _earth._a, _earth._f, lat, h, Z, X, M[7],
        M[8], _amodel, _GMmodel, _dzonal0, _corrmult, gamma0, gamma, fx,
