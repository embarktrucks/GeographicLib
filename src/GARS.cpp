...
void GARS::Forward(real lat, real lon, int prec, std::string& gars) {
    ...
    int x = int(floor(lon * m_)) - lonorig_ * m_,
        y = int(floor(lat * m_)) - latorig_ * m_, ilon = x * mult1_ / m_,
        ilat = y * mult1_ / m_;
    x -= ilon * m_ / mult1_;
    y -= ilat * m_ / mult1_;
    char gars1[maxlen_];
    ++ilon;
    for (int c = lonlen_; c--;) {
        gars1[c] = digits_[ilon % baselon_];
        ilon /= baselon_;
    }
    for (int c = latlen_; c--;) {
        gars1[lonlen_ + c] = letters_[ilat % baselat_];
        ilat /= baselat_;
    }
    if (prec > 0) {
        ilon = x / mult3_;
        ...
        if (prec > 1) {
            ...
            ilat = y % mult3_;
            gars1[baselen_ + 1] =
                digits_[mult3_ * (mult3_ - 1 - ilat) + ilon + 1];
        }
    }
...
void GARS::Reverse(const std::string& gars, real& lat, real& lon, int& prec,
                   bool centerp) {
    int len = int(gars.length());
    if (len >= 3 && toupper(gars[0]) == 'I' && toupper(gars[1]) == 'N' &&
        toupper(gars[2]) == 'V') {
        lat = lon = Math::NaN();
        return;
    }
    ...
    for (int c = 0; c < lonlen_; ++c) {
        ...
        if (k < 0) throw GeographicErr("GARS must start with 3 digits " + gars);
    ...
    real unit = mult1_, lat1 = ilat + latorig_ * unit,
    ...
    if (prec1 > 0) {
        ...
        if (prec1 > 1) {
            ...
            if (!(k >= 1 /* && k <= mult3_ * mult3_ */))
                throw GeographicErr("7th character in GARS must [1, 9] " +
                                    gars);
            --k;
            unit *= mult3_;
