...
Math::real AlbersEqualArea::txif(real tphi) const {
    ...
    real cphi2 = 1 / (1 + Math::sq(tphi)), sphi = tphi * sqrt(cphi2),
         ...
         es1m1 = (1 - es1) * sp1, es2m1a = _e2m * es2m1,
         es1p1 = sp1 / (1 + es1);
...
void AlbersEqualArea::Forward(real lon0, real lat, real lon, real& x, real& y,
    ...
    y = (_nrho0 * (_n0 != 0 ? (ctheta < 0 ? 1 - ctheta
...
void AlbersEqualArea::Reverse(real lon0, real x, real y, real& lat, real& lon,
    ...
    real nx = _k0 * _n0 * x, ny = _k0 * _n0 * y, y1 = _nrho0 - ny,
         den = Math::hypot(nx, y1) +
               _nrho0,  // 0 implies origin with polar aspect
        drho = den != 0
                   ? (_k0 * x * nx - 2 * _k0 * y * _nrho0 + _k0 * y * ny) / den
                   : 0,
         // dsxia = scxi0 * dsxi
        dsxia =
            -_scxi0 * (2 * _nrho0 + _n0 * drho) * drho / (Math::sq(_a) * _qZ),
         txi = (_txi0 + dsxia) /
               sqrt(max(1 - dsxia * (2 * _txi0 + dsxia), epsx2_)),
