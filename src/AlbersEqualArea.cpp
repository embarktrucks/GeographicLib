...
AlbersEqualArea::AlbersEqualArea(real a, real f, real sinlat1, real coslat1,
                                 real sinlat2, real coslat2, real k1)
  : eps_(numeric_limits<real>::epsilon()),
    epsx_(Math::sq(eps_)),
    epsx2_(Math::sq(epsx_)),
    tol_(sqrt(eps_)),
  ...
  if (coslat1 == 0 && coslat2 == 0 && sinlat1 * sinlat2 <= 0)
    throw GeographicErr("Standard latitudes cannot be opposite poles");
  Init(sinlat1, coslat1, sinlat2, coslat2, k1);
}

void AlbersEqualArea::Init(real sphi1, real cphi1, real sphi2, real cphi2,
                           real k1) {
  {
    real r;
  ...
  real tphi1 = sphi1 / cphi1, tphi2 = sphi2 / cphi2;
  ...
  if (polar || tphi1 == tphi2) {
    ...
    real tbet1 = _fm * tphi1, scbet12 = 1 + Math::sq(tbet1),
         tbet2 = _fm * tphi2, scbet22 = 1 + Math::sq(tbet2), txi1 = txif(tphi1),
         cxi1 = 1 / hyp(txi1), sxi1 = txi1 * cxi1, txi2 = txif(tphi2),
         cxi2 = 1 / hyp(txi2), sxi2 = txi2 * cxi2,
         dtbet2 = _fm * (tbet1 + tbet2), es1 = 1 - _e2 * Math::sq(sphi1),
         es2 = 1 - _e2 * Math::sq(sphi2),
         /*
            ...
            */
      dsxi =
        ((1 + _e2 * sphi1 * sphi2) / (es2 * es1) + Datanhee(sphi2, sphi1)) *
        Dsn(tphi2, tphi1, sphi2, sphi1) / (2 * _qx),
    ...
    for (int i = 0; i < 2 * numit0_ || GEOGRAPHICLIB_PANIC; ++i) {
      ...
      // v(tphi0) = (scbet0^2 * sphi0) - s * (1/qZ + scbet0^2 * sphi0 *
      // sxi0)
      //          = 0
      // Alt:
      ...
      real scphi02 = 1 + Math::sq(tphi0), scphi0 = sqrt(scphi02),
           // sphi0m = 1-sin(phi0) = 1/( sec(phi0) * (tan(phi0) +
           // sec(phi0)) )
        ...
        dg = _e2m * scphi02 * (1 + 2 * Math::sq(tphi0)) + _e2,
           D = sphi0m * (1 - _e2 * (1 + 2 * sphi0 * (1 + sphi0))) /
               (_e2m * (1 + sphi0)),
           // dD/dsphi0
        dD = -2 * (1 - _e2 * Math::sq(sphi0) * (2 * sphi0 + 3)) /
           ...
           B = (sphi0m * _e2m / (1 - _e2 * sphi0) *
                (atanhxm1(_e2 * Math::sq(sphi0m / (1 - _e2 * sphi0))) -
                 _e2 * sphi0m / _e2m)),
           // d(A+B)/dsphi0
...
const AlbersEqualArea& AlbersEqualArea::CylindricalEqualArea() {
  static const AlbersEqualArea cylindricalequalarea(
    Constants::WGS84_a(), Constants::WGS84_f(), real(0), real(1), real(0),
    real(1), real(1));
...
const AlbersEqualArea& AlbersEqualArea::AzimuthalEqualAreaNorth() {
  static const AlbersEqualArea azimuthalequalareanorth(
    Constants::WGS84_a(), Constants::WGS84_f(), real(1), real(0), real(1),
    real(0), real(1));
...
Math::real AlbersEqualArea::txif(real tphi) const {
  ...
  tphi *= s;
  real cphi2 = 1 / (1 + Math::sq(tphi)), sphi = tphi * sqrt(cphi2),
       es1 = _e2 * sphi, es2m1 = 1 - es1 * sphi, sp1 = 1 + sphi,
       es1m1 = (1 - es1) * sp1, es2m1a = _e2m * es2m1, es1p1 = sp1 / (1 + es1);
...
Math::real AlbersEqualArea::tphif(real txi) const {
  real tphi = txi, stol = tol_ * max(real(1), abs(txi));
  ...
  for (int i = 0; i < numit_ || GEOGRAPHICLIB_PANIC; ++i) {
    ...
    real txia = txif(tphi), tphi2 = Math::sq(tphi), scphi2 = 1 + tphi2,
         scterm = scphi2 / (1 + Math::sq(txia)),
         dtphi = (txi - txia) * scterm * sqrt(scterm) * _qx *
                 Math::sq(1 - _e2 * tphi2 / scphi2);
    tphi += dtphi;
    if (!(abs(dtphi) >= stol)) break;
  }
  return tphi;
}
...
void AlbersEqualArea::Forward(real lon0, real lat, real lon, real& x, real& y,
  ...
  cphi = max(epsx_, cphi);
  real lam = lon * Math::degree(), tphi = sphi / cphi, txi = txif(tphi),
       sxi = txi / hyp(txi),
...
void AlbersEqualArea::Reverse(real lon0, real x, real y, real& lat, real& lon,
                              real& gamma, real& k) const {
  y *= _sign;
  real nx = _k0 * _n0 * x, ny = _k0 * _n0 * y, y1 = _nrho0 - ny,
       den =
         Math::hypot(nx, y1) + _nrho0,  // 0 implies origin with polar aspect
    drho =
      den != 0 ? (_k0 * x * nx - 2 * _k0 * y * _nrho0 + _k0 * y * ny) / den : 0,
       // dsxia = scxi0 * dsxi
    dsxia = -_scxi0 * (2 * _nrho0 + _n0 * drho) * drho / (Math::sq(_a) * _qZ),
       txi =
         (_txi0 + dsxia) / sqrt(max(1 - dsxia * (2 * _txi0 + dsxia), epsx2_)),
       tphi = tphif(txi), theta = atan2(nx, y1),
