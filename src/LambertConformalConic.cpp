...
LambertConformalConic::LambertConformalConic(real a, real f, real stdlat,
                                             real k0)
...
LambertConformalConic::LambertConformalConic(real a, real f, real stdlat1,
                                             real stdlat2, real k1)
  : eps_(numeric_limits<real>::epsilon()),
    epsx_(Math::sq(eps_)),
    ahypover_(Math::digits() * log(real(numeric_limits<real>::radix)) + 2),
    _a(a),
...
LambertConformalConic::LambertConformalConic(real a, real f, real sinlat1,
                                             real coslat1, real sinlat2,
                                             real coslat2, real k1)
  : eps_(numeric_limits<real>::epsilon()),
    epsx_(Math::sq(eps_)),
    ahypover_(Math::digits() * log(real(numeric_limits<real>::radix)) + 2),
    _a(a),
...
void LambertConformalConic::Init(real sphi1, real cphi1, real sphi2, real cphi2,
                                 real k1) {
  {
    real r;
  ...
  real tphi1 = sphi1 / cphi1, tphi2 = sphi2 / cphi2, tphi0;
  //
  ...
  //
  real tbet1 = _fm * tphi1, scbet1 = hyp(tbet1), tbet2 = _fm * tphi2,
       scbet2 = hyp(tbet2);
  real scphi1 = 1 / cphi1, xi1 = Math::eatanhe(sphi1, _es), shxi1 = sinh(xi1),
       chxi1 = hyp(shxi1), tchi1 = chxi1 * tphi1 - shxi1 * scphi1,
       scchi1 = hyp(tchi1), scphi2 = 1 / cphi2, xi2 = Math::eatanhe(sphi2, _es),
       shxi2 = sinh(xi2), chxi2 = hyp(shxi2),
       tchi2 = chxi2 * tphi2 - shxi2 * scphi2, scchi2 = hyp(tchi2),
       psi1 = Math::asinh(tchi1);
  if (tphi2 - tphi1 != 0) {
    // Db(tphi2, tphi1)
    real num =
      Dlog1p(Math::sq(tbet2) / (1 + scbet2), Math::sq(tbet1) / (1 + scbet1)) *
    ...
    if (_n < 0.25)
      ...
      // Compute nc = cos(phi0) = sqrt((1 - n) * (1 + n)), evaluating 1 -
      // n carefully.  First write
      //
      ...
      // = Dlog1p(a2, a1) * (tchi2+scchi2 +
      // tchi1+scchi1)/(4*scbet1*scbet2)
      //   * fm * Q
      //
      // where
      ...
      // multiply by (tchi2 + scchi2 + tchi1 + scchi1)/(4*scbet1*scbet2) *
      // fm
      ...
      //     (1 - ((scbet2 + scbet1)/fm)/((scchi2 + scchi1)/D(tchi2,
      //     tchi1)))
      //   = tbm - tam
      // where
      ...
      // tam = (1 - ((scbet2+scbet1)/fm)/((scchi2+scchi1)/D(tchi2,
      // tchi1)))
      //
      // Let
      ...
      real
        ...
        dbet = (_e2 / _fm) *
               (1 / (scbet2 + _fm * scphi2) + 1 / (scbet1 + _fm * scphi1));
      ...
      //         D(tchi2, tchi1)
      real xiZ = Math::eatanhe(real(1), _es), shxiZ = sinh(xiZ),
           chxiZ = hyp(shxiZ),
           // These are differences not divided differences
           // dxiZ1 = xiZ - xi1; dshxiZ1 = shxiZ - shxi; dchxiZ1 = chxiZ -
           // chxi
        dxiZ1 = Deatanhe(real(1), sphi1) / (scphi1 * (tphi1 + scphi1)),
           ...
           // mu1 + mu2
        amu12 = (-scphi1 * dchxiZ1 + tphi1 * dshxiZ1 - scphi2 * dchxiZ2 +
                 tphi2 * dshxiZ2),
           // D(xi2, xi1)
  ...
  _nrho0 = polar ? 0 : _a * _k0 / _scbet0;
  {
    ...
    real sphi = -1, cphi = epsx_, tphi = sphi / cphi, scphi = 1 / cphi,
         shxi = sinh(Math::eatanhe(sphi, _es)),
         tchi = hyp(shxi) * tphi - shxi * scphi, scchi = hyp(tchi),
         psi = Math::asinh(tchi),
         dpsi = Dasinh(tchi, _tchi0, scchi, _scchi0) * (tchi - _tchi0);
    _drhomax =
      -_scale * (2 * _nc < 1 && dpsi != 0
...
const LambertConformalConic& LambertConformalConic::Mercator() {
  static const LambertConformalConic mercator(
    Constants::WGS84_a(), Constants::WGS84_f(), real(0), real(1));
  return mercator;
}

void LambertConformalConic::Forward(real lon0, real lat, real lon, real& x,
                                    real& y, real& gamma, real& k) const {
  ...
  cphi = max(epsx_, cphi);
  real lam = lon * Math::degree(), tphi = sphi / cphi, scbet = hyp(_fm * tphi),
       scphi = 1 / cphi, shxi = sinh(Math::eatanhe(sphi, _es)),
       tchi = hyp(shxi) * tphi - shxi * scphi, scchi = hyp(tchi),
       psi = Math::asinh(tchi), theta = _n * lam, stheta = sin(theta),
       ctheta = cos(theta),
       dpsi = Dasinh(tchi, _tchi0, scchi, _scchi0) * (tchi - _tchi0),
       drho =
         -_scale * (2 * _nc < 1 && dpsi != 0
  ...
  y = _nrho0 *
        (_n != 0
           ? (ctheta < 0 ? 1 - ctheta : Math::sq(stheta) / (1 + ctheta)) / _n
           : 0) -
      drho * ctheta;
  k = _k0 * (scbet / _scbet0) /
...
void LambertConformalConic::Reverse(real lon0, real x, real y, real& lat,
                                    real& lon, real& gamma, real& k) const {
  // From Snyder, we have
  //
  //        x = rho * sin(theta)
  ...
  if (_n == 0) drho = max(drho, -_drhomax);
  real tnm1 = _t0nm1 + _n * drho / _scale,
       dpsi = (den == 0 ? 0
  ...
  if (2 * _n <= 1) {
    // tchi = sinh(psi)
    real psi = _psi0 + dpsi, tchia = sinh(psi), scchi = hyp(tchia),
  ...
  // log(t) = -asinh(tan(chi)) = -psi
  gamma = atan2(nx, y1);
  real tphi = Math::tauf(tchi, _es), scbet = hyp(_fm * tphi), scchi = hyp(tchi),
