...
void LambertConformalConic::Init(real sphi1, real cphi1, real sphi2, real cphi2,
    ...
    real scphi1 = 1 / cphi1, xi1 = Math::eatanhe(sphi1, _es), shxi1 = sinh(xi1),
         ...
         scchi1 = hyp(tchi1), scphi2 = 1 / cphi2,
        ...
        if (_n < 0.25)
            ...
            real t;
            {
                real
                    ...
                    // t1 = scbet1 - tchi1
                    t1 = tchi1 < 0 ? scbet1 - tchi1
                                   : (s1 + 1) / (scbet1 + tchi1),
                    t2 = tchi2 < 0 ? scbet2 - tchi2
                                   : (s2 + 1) / (scbet2 + tchi2),
...
void LambertConformalConic::Forward(real lon0, real lat, real lon, real& x,
    ...
    cphi = max(epsx_, cphi);
    real lam = lon * Math::degree(), tphi = sphi / cphi,
         scbet = hyp(_fm * tphi), scphi = 1 / cphi,
         shxi = sinh(Math::eatanhe(sphi, _es)),
         tchi = hyp(shxi) * tphi - shxi * scphi, scchi = hyp(tchi),
    ...
    y = _nrho0 * (_n != 0 ? (ctheta < 0 ? 1 - ctheta
...
void LambertConformalConic::Reverse(real lon0, real x, real y, real& lat,
    ...
    real
        ...
        den =
            Math::hypot(nx, y1) + _nrho0,  // 0 implies origin with polar aspect
    ...
    // log(t) = -asinh(tan(chi)) = -psi
    gamma = atan2(nx, y1);
    real tphi = Math::tauf(tchi, _es), scbet = hyp(_fm * tphi),
         scchi = hyp(tchi), lam = _n != 0 ? gamma / _n : x / y1;
