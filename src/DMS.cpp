...
Math::real DMS::InternalDecode(const std::string& dmsa, flag& ind) {
    ...
    do {  // Executed once (provides the ability to break)
        ...
        if (end > beg &&
            (k = Utility::lookup(hemispheres_, dmsa[end - 1])) >= 0) {
            if (k >= 0) {
                if (ind1 != NONE) {
        ...
        while (p < end) {
            ...
            if ((k = Utility::lookup(digits_, x)) >= 0) {
                ...
                errormsg = "Illegal character " + Utility::str(x) +
                           " in DMS string " + dmsa.substr(beg, end - beg);
                break;
            }
        }
...
void DMS::DecodeLatLon(const std::string& stra, const std::string& strb,
    ...
    if (ia == ib)
        throw GeographicErr("Both " + stra + " and " + strb +
                            " interpreted as " +
