...
Math::real DMS::Decode(const std::string& dms, flag& ind) {
  ...
  replace(dmsa, "''", '"');             // '' -> "
  string::size_type beg = 0, end = unsigned(dmsa.size());
  while (beg < end && isspace(dmsa[beg])) ++beg;
  while (beg < end && isspace(dmsa[end - 1])) --end;
  ...
  for (string::size_type p = beg, pb; p < end; p = pb, ++i) {
    ...
    if (i == 0 && Utility::lookup(hemispheres_, dmsa[pa]) >= 0) ++pa;
    // Skip over initial sign (checking for it if i == 0)
    if (i > 0 || (pa < end && Utility::lookup(signs_, dmsa[pa]) >= 0)) ++pa;
    // Find next sign
...
Math::real DMS::InternalDecode(const std::string& dmsa, flag& ind) {
  ...
  do {  // Executed once (provides the ability to break)
    int sign = 1;
    unsigned beg = 0, end = unsigned(dmsa.size());
    flag ind1 = NONE;
    int k = -1;
    ...
    while (p < end) {
      ...
      if ((k = Utility::lookup(digits_, x)) >= 0) {
        ...
        if (pointseen) {
          errormsg =
            "Multiple decimal points in " + dmsa.substr(beg, end - beg);
          break;
        }
        pointseen = true;
        digcount = 1;
        ...
        if (unsigned(k) == npiece - 1) {
          errormsg = "Repeated " + string(components_[k]) + " component in " +
                     dmsa.substr(beg, end - beg);
          break;
        ...
        if (digcount > 0) {
          istringstream s(
            dmsa.substr(p - intcount - digcount - 1, intcount + digcount));
          s >> fcurrent;
          icurrent = 0;
        }
        ipieces[k] = icurrent;
    ...
    if (Utility::lookup(dmsindicators_, dmsa[p - 1]) < 0) {
      ...
      if (digcount > 0) {
        istringstream s(
          dmsa.substr(p - intcount - digcount, intcount + digcount));
        s >> fcurrent;
        icurrent = 0;
      }
    ...
    if (ipieces[1] >= 60 || fpieces[1] > 60) {
      errormsg =
        "Minutes " + Utility::str(fpieces[1]) + " not in range [0, 60)";
      break;
    }
    if (ipieces[2] >= 60 || fpieces[2] > 60) {
      errormsg =
        "Seconds " + Utility::str(fpieces[2]) + " not in range [0, 60)";
      break;
    }
    ind = ind1;
    ...
    return real(sign) *
           (fpieces[2] != 0
              ...
              : (fpieces[1] != 0 ? (60 * fpieces[0] + fpieces[1]) / 60
                                 : fpieces[0]));
  } while (false);
...
void DMS::DecodeLatLon(const std::string& stra, const std::string& strb,
                       real& lat, real& lon, bool longfirst) {
  real a, b;
  flag ia, ib;
  a = Decode(stra, ia);
  b = Decode(strb, ib);
  ...
  if (ia == ib)
    throw GeographicErr("Both " + stra + " and " + strb + " interpreted as " +
                        (ia == LATITUDE ? "latitudes" : "longitudes"));
  real lat1 = ia == LATITUDE ? a : b, lon1 = ia == LATITUDE ? b : a;
  if (abs(lat1) > 90)
...
string DMS::Encode(real angle, component trailing, unsigned prec, flag ind,
  ...
  real scale = 1;
  for (unsigned i = 0; i < unsigned(trailing); ++i) scale *= 60;
  for (unsigned i = 0; i < prec; ++i) scale *= 10;
  if (ind == AZIMUTH) angle -= floor(angle / 360) * 360;
  int sign = angle < 0 ? -1 : 1;
  angle *= sign;

  // Break off integer part to preserve precision in manipulation of
  // fractional part.
  real idegree = floor(angle), fdegree = (angle - idegree) * scale + real(0.5);
  {
  ...
  for (unsigned i = 1; i <= unsigned(trailing); ++i) {
    real ip = floor(pieces[i - 1]), fp = pieces[i - 1] - ip;
    pieces[i] = fp * 60;
    pieces[i - 1] = ip;
  }
  pieces[0] += idegree;
  ostringstream s;
  s << fixed << setfill('0');
  if (ind == NONE && sign < 0) s << '-';
  switch (trailing) {
    case DEGREE:
      if (ind != NONE) s << setw(1 + min(int(ind), 2) + prec + (prec ? 1 : 0));
      s << Utility::str(pieces[0], prec);
      // Don't include degree designator (d) if it is the trailing
      // component.
      break;
    default:
      if (ind != NONE) s << setw(1 + min(int(ind), 2));
      s << int(pieces[0])
        << (dmssep ? dmssep : char(tolower(dmsindicators_[0])));
      switch (trailing) {
        case MINUTE:
          s << setw(2 + prec + (prec ? 1 : 0)) << Utility::str(pieces[1], prec);
          if (!dmssep) s << char(tolower(dmsindicators_[1]));
          break;
        case SECOND:
          s << setw(2) << int(pieces[1])
            << (dmssep ? dmssep : char(tolower(dmsindicators_[1])))
            << setw(2 + prec + (prec ? 1 : 0)) << Utility::str(pieces[2], prec);
          if (!dmssep) s << char(tolower(dmsindicators_[2]));
          break;
        default:
          break;
      }
