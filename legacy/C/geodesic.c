...
static real atan2dx(real y, real x) {
    ...
    switch (q) {
            /* Note that atan2d(-0.0, 1.0) will return -0.  However, we expect
             * that atan2d will not be called with y = -0.  If need be, include
             *
...
void geod_init(struct geod_geodesic* g, real a, real f) {
    ...
    g->etol2 = 0.1 * tol2 /
               sqrt(maxx((real)(0.001), fabs(g->f)) *
                    minx((real)(1), 1 - g->f / 2) / 2);
...
real geod_genposition(const struct geod_geodesicline* l, unsigned flags,
    ...
    if (flags & GEOD_ARCMODE) {
        ...
        if (fabs(l->f) > 0.01) {
             ...
             * correct sig12 with 1 Newton iteration.  The following table shows
             * the approximate maximum error for a = WGS_a() and various f
             * relative to GeodesicExact. erri = the error in the inverse
             * solution (nm) errd = the error in the direct solution (series
             * only) (nm) errda = the error in the direct solution (series + 1
             * Newton) (nm)
             *
             *       f     erri  errd errda
             *     -1/5    12e6 1.2e9  69e6
    ...
    if (outmask & GEOD_DISTANCE)
        s12 = flags & GEOD_ARCMODE ? l->b * ((1 + l->A1m1) * sig12 + AB1)
                                   : s12_a12;
    ...
    if (outmask & GEOD_LONGITUDE) {
        ...
        lam12 = omg12 + l->A3c * (sig12 + (SinCosSeries(TRUE, ssig2, csig2,
                                                        l->C3a, nC3 - 1) -
                                           l->B31));
        lon12 = lam12 / degree;
    ...
    if (outmask & (GEOD_REDUCEDLENGTH | GEOD_GEODESICSCALE)) {
        ...
        if (outmask & GEOD_REDUCEDLENGTH)
            ...
            m12 = l->b *
                  ((dn2 * (l->csig1 * ssig2) - l->dn1 * (l->ssig1 * csig2)) -
                   l->csig1 * csig2 * J12);
    ...
    if (outmask & GEOD_AREA) {
        ...
        if (l->calp0 == 0 || l->salp0 == 0) {
             ...
             * csig1*csig2) If csig12 > 0, write csig1 - csig2 = ssig12 * (csig1
             * * ssig12 / (1 + csig12) + ssig1) else csig1 - csig2 = csig1 * (1
             * - csig12) + ssig12 * ssig1 No need to normalize */
            salp12 =
                l->calp0 * l->salp0 *
                (csig12 <= 0
...
static real geod_geninverse_int(const struct geod_geodesic* g, real lat1,
    ...
    if (meridian) {
        ...
        Lengths(g, g->n, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1,
                cbet2, &s12x, &m12x, 0, outmask & GEOD_GEODESICSCALE ? &M12 : 0,
    ...
    if (!meridian && sbet1 == 0 && /* and sbet2 == 0 */
        ...
        real dnm = 0;
        sig12 =
            InverseStart(g, sbet1, cbet1, dn1, sbet2, cbet2, dn2, lam12, slam12,
        ...
        if (sig12 >= 0) {
            ...
            for (tripn = FALSE, tripb = FALSE; numit < maxit2; ++numit) {
                ...
                real dv = 0,
                     v = Lambda12(g, sbet1, cbet1, dn1, sbet2, cbet2, dn2,
                ...
                if (v > 0 &&
                    (numit > maxit1 || calp1 / salp1 > calp1b / salp1b)) {
                    salp1b = salp1;
                    calp1b = calp1;
                } else if (v < 0 && (numit > maxit1 ||
            ...
            Lengths(g, eps, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1,
                    cbet2, &s12x, &m12x, 0,
...
real geod_geninverse(const struct geod_geodesic* g, real lat1, real lon1,
    ...
    real salp1, calp1, salp2, calp2,
        a12 =
            geod_geninverse_int(g, lat1, lon1, lat2, lon2, ps12, &salp1, &calp1,
...
void geod_inverseline(struct geod_geodesicline* l,
    ...
    real salp1, calp1,
        a12 = geod_geninverse_int(g, lat1, lon1, lat2, lon2, 0, &salp1, &calp1,
                                  0, 0, 0, 0, 0, 0),
...
real InverseStart(const struct geod_geodesic* g, real sbet1, real cbet1,
    ...
    if (shortline && ssig12 < g->etol2) {
        ...
        sig12 = atan2(ssig12, csig12);
                   ...
                   (real)(0.1) || /* No astroid calc if too eccentric */
               csig12 >= 0 ||
...
void A3coeff(struct geod_geodesic* g) {
    ...
    for (j = nA3 - 1; j >= 0; --j) { /* coeff of eps^j */
        int m =
            nA3 - j - 1 < j ? nA3 - j - 1 : j; /* order of polynomial in n */
...
void geod_polygon_addedge(const struct geod_geodesic* g, struct geod_polygon* p,
    ...
    if (p->num) { /* Do nothing is num is zero */
        real lat, lon,
            S12 = 0; /* Initialize S12 to stop Visual Studio warning */
        geod_gendirect(g, p->lat, p->lon, azi, GEOD_LONG_UNROLL, s, &lat, &lon,
                       0, 0, 0, 0, 0, p->polyline ? 0 : &S12);
        accadd(p->P, s);
        if (!p->polyline) {
            accadd(p->A, S12);
...
unsigned geod_polygon_testpoint(const struct geod_geodesic* g,
    ...
    for (i = 0; i < (p->polyline ? 1 : 2); ++i) {
        ...
        geod_geninverse(g, i == 0 ? p->lat : lat, i == 0 ? p->lon : lon,
                        i != 0 ? p->lat0 : lat, i != 0 ? p->lon0 : lon, &s12, 0,
...
unsigned geod_polygon_testedge(const struct geod_geodesic* g,
    ...
    crossings = p->crossings;
    {
        ...
        geod_geninverse(g, lat, lon, p->lat0, p->lon0, &s12, 0, 0, 0, 0, 0,
                        &S12);
        perimeter += s12;
        tempsum += S12;
