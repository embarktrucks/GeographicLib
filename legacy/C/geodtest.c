...
double testcases[20][12] = {
  ...
  {-21.97856, 142.59065, -32.44456876433189, 41.84138, 98.56635,
   -41.84359951440466, 8394328.894657671, 75.62930491011522, 6161154.5773110616,
  ...
  {-68.82867, -74.28391, 93.774347763114881, -50.63005, -8.36685,
   34.65564085411343, 3956936.926063544, 35.572254987389284, 3708890.9544062657,
  ...
  {-41.22777, 122.32875, 14.285113402275739, -7.57291, 130.37946,
   10.805303085187369, 3812686.035106021, 34.34330804743883, 3588703.8812128856,
...
static int testdirect() {
  ...
  for (i = 0; i < ncases; ++i) {
    ...
    M21 = testcases[i][10];
    S12 = testcases[i][11];
    a12a = geod_gendirect(&g, lat1, lon1, azi1, flags, s12, &lat2a, &lon2a,
                          &azi2a, 0, &m12a, &M12a, &M21a, &S12a);
...
static int testarcdirect() {
  ...
  for (i = 0; i < ncases; ++i) {
    ...
    M21 = testcases[i][10];
    S12 = testcases[i][11];
    geod_gendirect(&g, lat1, lon1, azi1, flags, a12, &lat2a, &lon2a, &azi2a,
                   &s12a, &m12a, &M12a, &M21a, &S12a);
    result += assertEquals(lat2, lat2a, 1e-13);
    result += assertEquals(lon2, lon2a, 1e-13);
    result += assertEquals(azi2, azi2a, 1e-13);
    result += assertEquals(s12, s12a, 1e-8);
...
static int GeodSolve1() {
  ...
  int result = 0;
  geod_init(&g, wgs84_a, wgs84_f);
  geod_direct(&g, 40.63972222, -73.77888889, 53.5, 5850e3, &lat2, &lon2, &azi2);
...
static int GeodSolve4() {
  ...
  int result = 0;
  geod_init(&g, wgs84_a, wgs84_f);
  geod_inverse(&g, 36.493349428792, 0, 36.49334942879201, .0000008, &s12, 0, 0);
...
static int GeodSolve6() {
  ...
  int result = 0;
  geod_init(&g, wgs84_a, wgs84_f);
  geod_inverse(&g, 88.202499451857, 0, -88.202499451857, 179.981022032992859592,
               &s12, 0, 0);
  result += assertEquals(s12, 20003898.214, 0.5e-3);
  geod_inverse(&g, 89.262080389218, 0, -89.262080389218, 179.992207982775375662,
               &s12, 0, 0);
  result += assertEquals(s12, 20003925.854, 0.5e-3);
  geod_inverse(&g, 89.333123580033, 0, -89.333123580032997687,
               179.99295812360148422, &s12, 0, 0);
...
static int GeodSolve9() {
  ...
  int result = 0;
  geod_init(&g, wgs84_a, wgs84_f);
  geod_inverse(&g, 56.320923501171, 0, -56.320923501171, 179.664747671772880215,
               &s12, 0, 0);
...
static int GeodSolve10() {
  ...
  int result = 0;
  geod_init(&g, wgs84_a, wgs84_f);
  geod_inverse(&g, 52.784459512564, 0, -52.784459512563990912,
               179.634407464943777557, &s12, 0, 0);
...
static int GeodSolve11() {
  ...
  int result = 0;
  geod_init(&g, wgs84_a, wgs84_f);
  geod_inverse(&g, 48.522876735459, 0, -48.52287673545898293,
               179.599720456223079643, &s12, 0, 0);
...
static int GeodSolve15() {
  ...
  int result = 0;
  geod_init(&g, 6.4e6, -1 / 150.0);
  geod_gendirect(&g, 1, 2, 3, 0, 4, 0, 0, 0, 0, 0, 0, 0, &S12);
...
static int GeodSolve17() {
  ...
  unsigned flags = GEOD_LONG_UNROLL;
  geod_init(&g, wgs84_a, wgs84_f);
  geod_gendirect(&g, 40, -75, -10, flags, 2e7, &lat2, &lon2, &azi2, 0, 0, 0, 0,
                 0);
...
static int GeodSolve61() {
  ...
  unsigned flags = GEOD_LONG_UNROLL;
  geod_init(&g, wgs84_a, wgs84_f);
  geod_gendirect(&g, 45, 0, -0.000000000000000003, flags, 1e7, &lat2, &lon2,
                 &azi2, 0, 0, 0, 0, 0);
...
static int GeodSolve65() {
  ...
  a12 = geod_genposition(&l, flags, 1e7, &lat2, &lon2, &azi2, &s12, &m12, &M12,
                         &M21, &S12);
  ...
  a12 = geod_genposition(&l, flags, 2e7, &lat2, &lon2, &azi2, &s12, &m12, &M12,
                         &M21, &S12);
...
static int GeodSolve73() {
  ...
  int result = 0;
  geod_init(&g, wgs84_a, wgs84_f);
  geod_direct(&g, 90, 10, 180, -1e6, &lat2, &lon2, &azi2);
...
static int GeodSolve74() {
  ...
  int result = 0;
  geod_init(&g, wgs84_a, wgs84_f);
  a12 = geod_geninverse(&g, 54.1589, 15.3872, 54.1591, 15.3877, &s12, &azi1,
...
static int GeodSolve76() {
  ...
  int result = 0;
  geod_init(&g, wgs84_a, wgs84_f);
  geod_inverse(&g, -(41 + 19 / 60.0), 174 + 49 / 60.0, 40 + 58 / 60.0,
               -(5 + 30 / 60.0), &s12, &azi1, &azi2);
...
int main() {
  ...
  if ((i = testinverse())) {
    ++n;
    printf("testinverse fail: %d\n", i);
  }
  if ((i = testdirect())) {
    ++n;
    printf("testdirect fail: %d\n", i);
  }
  if ((i = testarcdirect())) {
    ++n;
    printf("testarcdirect fail: %d\n", i);
  }
  if ((i = GeodSolve0())) {
    ++n;
    printf("GeodSolve0 fail: %d\n", i);
  }
  if ((i = GeodSolve1())) {
    ++n;
    printf("GeodSolve1 fail: %d\n", i);
  }
  if ((i = GeodSolve2())) {
    ++n;
    printf("GeodSolve2 fail: %d\n", i);
  }
  if ((i = GeodSolve4())) {
    ++n;
    printf("GeodSolve4 fail: %d\n", i);
  }
  if ((i = GeodSolve5())) {
    ++n;
    printf("GeodSolve5 fail: %d\n", i);
  }
  if ((i = GeodSolve6())) {
    ++n;
    printf("GeodSolve6 fail: %d\n", i);
  }
  if ((i = GeodSolve9())) {
    ++n;
    printf("GeodSolve9 fail: %d\n", i);
  }
  if ((i = GeodSolve10())) {
    ++n;
    printf("GeodSolve10 fail: %d\n", i);
  }
  if ((i = GeodSolve11())) {
    ++n;
    printf("GeodSolve11 fail: %d\n", i);
  }
  if ((i = GeodSolve12())) {
    ++n;
    printf("GeodSolve12 fail: %d\n", i);
  }
  if ((i = GeodSolve14())) {
    ++n;
    printf("GeodSolve14 fail: %d\n", i);
  }
  if ((i = GeodSolve15())) {
    ++n;
    printf("GeodSolve15 fail: %d\n", i);
  }
  if ((i = GeodSolve17())) {
    ++n;
    printf("GeodSolve17 fail: %d\n", i);
  }
  if ((i = GeodSolve26())) {
    ++n;
    printf("GeodSolve26 fail: %d\n", i);
  }
  if ((i = GeodSolve28())) {
    ++n;
    printf("GeodSolve28 fail: %d\n", i);
  }
  if ((i = GeodSolve33())) {
    ++n;
    printf("GeodSolve33 fail: %d\n", i);
  }
  if ((i = GeodSolve55())) {
    ++n;
    printf("GeodSolve55 fail: %d\n", i);
  }
  if ((i = GeodSolve59())) {
    ++n;
    printf("GeodSolve59 fail: %d\n", i);
  }
  if ((i = GeodSolve61())) {
    ++n;
    printf("GeodSolve61 fail: %d\n", i);
  }
  if ((i = GeodSolve65())) {
    ++n;
    printf("GeodSolve65 fail: %d\n", i);
  }
  if ((i = GeodSolve67())) {
    ++n;
    printf("GeodSolve67 fail: %d\n", i);
  }
  if ((i = GeodSolve71())) {
    ++n;
    printf("GeodSolve71 fail: %d\n", i);
  }
  if ((i = GeodSolve73())) {
    ++n;
    printf("GeodSolve73 fail: %d\n", i);
  }
  if ((i = GeodSolve74())) {
    ++n;
    printf("GeodSolve74 fail: %d\n", i);
  }
  if ((i = GeodSolve76())) {
    ++n;
    printf("GeodSolve76 fail: %d\n", i);
  }
  if ((i = GeodSolve78())) {
    ++n;
    printf("GeodSolve78 fail: %d\n", i);
  }
  if ((i = Planimeter0())) {
    ++n;
    printf("Planimeter0 fail: %d\n", i);
  }
  if ((i = Planimeter5())) {
    ++n;
    printf("Planimeter5 fail: %d\n", i);
  }
  if ((i = Planimeter6())) {
    ++n;
    printf("Planimeter6 fail: %d\n", i);
  }
  if ((i = Planimeter12())) {
    ++n;
    printf("Planimeter12 fail: %d\n", i);
  }
  if ((i = Planimeter13())) {
    ++n;
    printf("Planimeter13 fail: %d\n", i);
  }
  return n;
}
