...
static int testinverse() {
    ...
    for (i = 0; i < ncases; ++i) {
        ...
        M21 = testcases[i][10];
        S12 = testcases[i][11];
        a12a = geod_geninverse(&g, lat1, lon1, lat2, lon2, &s12a, &azi1a,
                               &azi2a, &m12a, &M12a, &M21a, &S12a);
...
static int GeodSolve1() {
    ...
    int result = 0;
    geod_init(&g, wgs84_a, wgs84_f);
    geod_direct(&g, 40.63972222, -73.77888889, 53.5, 5850e3, &lat2, &lon2,
                &azi2);
...
static int GeodSolve4() {
    ...
    int result = 0;
    geod_init(&g, wgs84_a, wgs84_f);
    geod_inverse(&g, 36.493349428792, 0, 36.49334942879201, .0000008, &s12, 0,
                 0);
...
static int GeodSolve6() {
    ...
    int result = 0;
    geod_init(&g, wgs84_a, wgs84_f);
    geod_inverse(&g, 88.202499451857, 0, -88.202499451857,
                 179.981022032992859592, &s12, 0, 0);
    result += assertEquals(s12, 20003898.214, 0.5e-3);
    geod_inverse(&g, 89.262080389218, 0, -89.262080389218,
                 179.992207982775375662, &s12, 0, 0);
...
static int GeodSolve9() {
    ...
    int result = 0;
    geod_init(&g, wgs84_a, wgs84_f);
    geod_inverse(&g, 56.320923501171, 0, -56.320923501171,
                 179.664747671772880215, &s12, 0, 0);
...
static int GeodSolve17() {
    ...
    unsigned flags = GEOD_LONG_UNROLL;
    geod_init(&g, wgs84_a, wgs84_f);
    geod_gendirect(&g, 40, -75, -10, flags, 2e7, &lat2, &lon2, &azi2, 0, 0, 0,
                   0, 0);
...
static int GeodSolve65() {
    ...
    a12 = geod_genposition(&l, flags, 1e7, &lat2, &lon2, &azi2, &s12, &m12,
                           &M12, &M21, &S12);
    ...
    a12 = geod_genposition(&l, flags, 2e7, &lat2, &lon2, &azi2, &s12, &m12,
                           &M12, &M21, &S12);
...
static int GeodSolve67() {
    ...
    result += assertEquals(lon2, -180.00000, 0.5e-5);
    result += assertEquals(azi2, -0.00000, 0.5e-5);
    geod_genposition(&l, flags, 0.5 * l.s13, &lat2, &lon2, &azi2, 0, 0, 0, 0,
                     0);
