 ...
 * \brief Header for geographic_lib::NearestNeighbor class
 *
...
#include <iostream>
#include <sstream>
// Only for GEOGRAPHICLIB_STATIC_ASSERT and geographic_lib::GeographicErr
...
namespace geographic_lib {
   ...
   * way on the rest of geographic_lib (the only dependency is through the use
   * of GEOGRAPHICLIB_STATIC_ASSERT and geographic_lib::GeographicErr for
   * handling run-time and compile-time exceptions).  Therefore, it is easy to
   * extract this class from the rest of geographic_lib and use it as a
  ...
  public:
    ...
    dist_t Search(const std::vector<pos_t>& pts, const distfun_t& dist,
      ...
      if (_numpoints != int(pts.size()))
          throw geographic_lib::GeographicErr("pts array has wrong size");
    ...
    void Load(std::istream& is, bool bin = true) {
      ...
      if (bin) {
        ...
        if (!(std::strcmp(id, "NearestNeighbor_") == 0))
          throw geographic_lib::GeographicErr("Bad ID");
        ...
        if (!( is >> version1 >> realspec >> bucket >> numpoints >> treesize
               >> cost ))
          throw geographic_lib::GeographicErr("Bad header");
      }
      if (!( version1 == version ))
        throw geographic_lib::GeographicErr("Incompatible version");
      if (!( realspec == std::numeric_limits<dist_t>::digits *
             (std::numeric_limits<dist_t>::is_integer ? -1 : 1) ))
        throw geographic_lib::GeographicErr("Different dist_t types");
      if (!( 0 <= bucket && bucket <= maxbucket ))
        throw geographic_lib::GeographicErr("Bad bucket size");
      if (!( 0 <= treesize && treesize <= numpoints ))
        throw
          geographic_lib::GeographicErr("Bad number of points or tree size");
      if (!( 0 <= cost ))
        throw geographic_lib::GeographicErr("Bad value for cost");
      std::vector<Node> tree;
      ...
      for (int i = 0; i < treesize; ++i) {
        ...
        if (bin) {
          ...
          if (!( is >> node.index ))
            throw geographic_lib::GeographicErr("Bad index");
          if (node.index >= 0) {
            for (int l = 0; l < 2; ++l) {
              if (!( is >> node.data.lower[l] >> node.data.upper[l]
                     >> node.data.child[l] ))
                throw geographic_lib::GeographicErr("Bad node data");
            }
          } else {
            ...
            for (int l = 0; l < bucket; ++l) {
              if (!( is >> node.leaves[l] ))
                throw geographic_lib::GeographicErr("Bad leaf data");
            }
            for (int l = bucket; l < maxbucket; ++l)
              node.leaves[l] = 0;
          }
      ...
      void Check(int numpoints, int treesize, int bucket) const {
        ...
        if (index >= 0) {
          if (!( -1 <= data.child[0] && data.child[0] < treesize &&
            ...
            throw geographic_lib::GeographicErr("Bad child pointers");
          if (!( 0 <= data.lower[0] && data.lower[0] <= data.upper[0] &&
                 data.upper[0] <= data.lower[1] &&
                 data.lower[1] <= data.upper[1] ))
            throw geographic_lib::GeographicErr("Bad bounds");
        } else {
  ...
  GEOGRAPHICLIB_HAVE_BOOST_SERIALIZATION
    ...
    template<class Archive> void save(Archive& ar, const unsigned) const {
      ...
      // undefined reference to geographic_lib::NearestNeighbor<...>::version.
...
namespace std {
  ...
  /**
   * Swap two geographic_lib::NearestNeighbor objects.
   *
   ...
   * @param[in,out] a the first geographic_lib::NearestNeighbor to swap.
   * @param[in,out] b the second geographic_lib::NearestNeighbor to swap.
   **********************************************************************/
  template <typename dist_t, typename pos_t, class distfun_t>
  void swap(geographic_lib::NearestNeighbor<dist_t, pos_t, distfun_t>& a,
            geographic_lib::NearestNeighbor<dist_t, pos_t, distfun_t>& b) {
    a.swap(b);
  }
